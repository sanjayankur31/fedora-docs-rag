[Fedora Magazine](https://fedoramagazine.org) is a *WordPress-based*
site which delivers all the news of the Fedora Community. (It replaced
the older publication, Fedora Weekly News.)
...
We really need your help on the Magazine. Consider [writing an
article](contributing.xml). There is also a detailed guide on how to
make a post on the magazine.

We also have regular [editorial board meetings](editorial-meetings.xml)
on chat.fedoraproject.org. See the [Meeting and Publication
Schedule](https://discussion.fedoraproject.org/t/meeting-and-publication-schedule/34554)
for exactly when and where.

We also have two areas on the discussion.fedoraproject.org forum for
Fedora Magazine topics. One section (under the *Team Workflows*
category) is reserved just for article proposals. When you start a new
topic there, the post should be pre-populated with a simple outline that
you can complete to make a new proposal. The other area is for anything
else related to Fedora Magazine. These can be posts about the site
...
![fedora magazine workflow](fedora-magazine-workflow.jpg)

Content on the Fedora Magazine is created using a five-step workflow:

1.  [Article Proposal](workflow-article-proposal.xml) by Anyone

2.  [Clarification](workflow-clarification.xml) by Editors

3.  [Writing](writing-an-article.xml) by a Writer
...
progresses to *in progress*, *review*, *to edit*, and finally ending up
being *scheduled*. Each step in the workflow has its own page in this
documentation that provides guidance.

Article proposals in are initiated and discussed under the *Team
Workflows* section of the [Fedora Magazine Discourse
forum](https://discussion.fedoraproject.org/c/workflows/magazine/57).
Everyone is welcome to submit one!

Articles in all other states are represented as cards in the [Kanban
section of the Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles).
...
Proposals can be ideas or specific pitches (an example is below).
Proposing an article doesn't automatically mean the same person needs to
write it. Having a good set of ideas helps writers to write what they
know is relevant for the Magazine.

Join the community on the [Fedora Magazine Discourse
...
If this is your first proposal, and you also plan to write an article,
you can help us speed the process up by logging into our [Kanban section
of the Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles) ---
Select the *Log In* button to log in with your Fedora Account. That is
required before an article can be assigned to you.
::::

*The following article serves as an example outline for proposing a new
pitch for the Fedora Magazine. This example was used for the [Never
...
**Next step:** [Clarification](workflow-clarification.xml)

![fedora magazine workflow 2](fedora-magazine-workflow-2.jpg)

The Editorial Board reviews each proposal and creates a specific article
description.

Proposals might be discussed with the person who proposed them, with
subject matter experts, and anyone who wants (or has the technical
ability to) give an input. We encourage anyone in the community to
provide feedback to the proposals!

Proposals are submitted to the [Fedora Magazine Discourse
forum](https://discussion.fedoraproject.org/c/workflows/magazine/57) as
topics, and discussion happens right there.
...
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles) in
the *ideas* column.

When the card is created, and the author is known, the owner is assigned
in the *Assignee* field under the METADATA column. This ensures the
author is emailed when the card status changes. This operation is
performed by the editor.

This card now represents a specific article and goes through all the
...
Anyone with good writing skills is welcome to write an article for the
Fedora Magazine!

*Writing an article requires write access to our [Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles) (so
you can indicate progress and exchange information with the editors) and
to our [Wordpress instance](https://fedoramagazine.org/admin) (where the
articles are written). Read the [Getting Access](getting-access.xml)
...
the editors regarding an article, simply let them know in your ongoing
conversation.

The editors will assign the article to you and move it to *in-progress*
in the [Fedora Magazine Kanban
...
[Here's a template you can
use](https://fedoramagazine.org/how-to-structure-your-article/) to
structure your article well, and make it as useful as possible to
readers. Make sure you set the article status from *Pitch* to *Draft*
once you receive approval.

As you write your draft, try to understand and follow the [style,
grammar, and SEO guidelines](writing-guidelines.xml) for the Magazine.
Don't skip this step! The guidelines tell you how to use markup, write
better sentences, and get your article ranked well in search engines.
Also, if you write a better article, the editors are more likely to
publish it sooner.

When you feel you are done writing the article and it is ready for
review, state that in a comment on your card in Pagure. The editors will
move it to *review* in the Kanban board. Please make certain you include
a link to the WordPress preview for your article in the comments on the
card so the Editors can access it.

And that's it!

Please note that in some cases the editors might require a change in
your article. In that case, you'll receive feedback in the card, and it
will be moved back to the *in-progress* column. Please make the
requested change and let the editors know in the comments. They will
move it back to *review*.
...
Articles that are ready for review are in the *review* column in the
[Kanban board of the Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles).

If the article is good to go, a member of the Editorial Board simply
moves it to the *to-edit* column in the [Kanban
board](https://pagure.io/fedora-magazine-newsroom/boards/articles).

If the article needs changes, a member of the Editorial Board provides
feedback in the comments section of the respective card, and moves it
back to the *in-progress* column in the [Kanban
board](https://pagure.io/fedora-magazine-newsroom/boards/articles).

**Previous step:** [Writing](writing-an-article.xml)

**Next step:** [Edit & Image](workflow-edit-image.xml)

![fedora magazine workflow 5](fedora-magazine-workflow-5.jpg)

Before an article can be published, it needs to be edited, and it needs
a cover image.

Members of the Editorial Board provide both of those.

Articles that require editing are in the *to-edit* column in the [Kanban
board of the Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles).
...
Articles that require a cover image are in the *to-edit* column in the
[Kanban board of the Fedora Magazine
Pagure](https://pagure.io/fedora-magazine-newsroom/boards/articles).
...
Just propose your idea on our [Fedora Magazine Discourse
forum](https://discussion.fedoraproject.org/new-topic?category=workflows/magazine).
...
Writing](writing-an-article.xml) page for guidance.**
...
This is so the account is known to the system and you can edit card
meta-data. If an editor has already created a card for your idea, you
should be able to assign the card to yourself. You will have to request
that the card's overall status (i.e. the column that it is in on the
...
The Fedora Magazine editors monitor this Discourse forum and will be
able to help you and answer any Fedora Magazine related questions.

These tips will help your article sail through editing. Be kind to your
editor(s), and read this before you start.
...
5.  **Use sentence case for the post title and heading titles.** Don't
...
9.  **Be careful of gerunds (-ing words).** They usually indicate
...
10. **Avoid unnecessary future tense.** Unless you're actually talking
...
11. **Avoid too much use of the verb *to be* in sentences.** Too much
...
12. **Use standard US English for spelling and other international
    differences.** US English is the lingua franca for the Fedora
    Project overall.

13. **Have a smooth flow from general information to specific
...
users to do. Remember that thousands of readers trust Fedora Magazine to
...
9.  **Use a fresh, standard Fedora Workstation**, not your personal
...
10. **Set screen resolution at a reasonable but not too high
    resolution**, such as 1280x960 or 1280x800.

11. If you're only showing a browser window, make it fairly large on the
    screen and **screenshot only the browser**. (Hint: you can use the
    Screenshot app for this!)

12. If you're only showing an application, **use the default size of the
    app to screenshot it**.

13. **Upload and use that original media in your article.** If the shot
...
14. **Use a simple, relevant title.** Preferably this should be a call
...
15. **Provide a featured image.** Look at [the information
    ...
    match the style of other images. Use the assets provided [in the
    featured image
...
16. **Use good SEO practices.** Use the SEO plugin provided to maximize
...
17. **Use the Edit as HTML option when pasting into preformatted
...
social media, to attract more viewers.
...
Watch the [Creating Featured Images Workshop
video](https://youtu.be/F6Nf1gsR9S4) on our YouTube channel for a visual
demo of the guidelines and instructions below, plus some additional
Inkscape tips.
...
Open a Terminal. (You'll type commands into the terminal for some of the
following steps.)

Install the git and inkscape packages if you don't have them installed
already:

\$ sudo dnf install git inkscape

Fork the image repository at
[fedoramagazine-images](https://pagure.io/fedoramagazine-images) .
...
other SVGs for guidance. If you don't have experience with these images,
try to stick with fonts and motifs that you see in recent banners. But
also feel free to experiment! For instance, use photographs (freely
licensed, such as CC-BY or CC-BY-SA) with a text overlay for a nice
look. [Unsplash.com](https://unsplash.com/) has a large library of
images in the public domain you can use to get started.

It makes sense to reuse an old image if there is a thematic link from
the previous article to the new one. Try to vary something about the
image, such as the text. If the background is abstract (not a photo),
consider rebalancing the color.
...
You can find compatibly licensed images at
[Unsplash](https://unsplash.com), or you can try other image search
engines. Be sure to choose only images with CC0, CC-BY, or CC-BY-SA
licenses. (Commercial and derivative uses must be permissible.)

Before you import your background to the SVG, scale and trim it using
GIMP or another image editor to an 1890x800 size, and a JPG quality of
about 75-80%. This keeps the SVG from being too large --- it will reduce
the size from many MB to just a few hundred KB.

The SVG template contains guidelines to indicate what portion of the
cover image's edge might get clipped when the article is shared on
various social media sites. The portion that might get clipped is about
10% of the left and right sides of the image and about 13% of the top
and bottom. In the SVG template, the guidelines provide margins of
exactly 183 pixels on the left and right and 107 pixels on the top and
bottom for an image that is sized to 1890x800 pixels. This leaves an
interior region of 1524x586 pixels within which it is safe to place text
without concern that it will get clipped when the article is shared out
to other social media sites.

There are a few fonts that are commonly used in featured images. You are
encouraged to use these fonts in your images to help featured images fit
into the general "theme" that other images follow. They are all freely
available at the links provided. The fonts we commonly use on the Fedora
...
3.  [Oswald](https://www.fontsquirrel.com/fonts/oswald) (for a more
    technical feel)

4.  [Josefin Slab](https://www.fontsquirrel.com/fonts/josefin-slab)
    (friendly with serifs)

5.  [Zilla Slab](https://www.fontsquirrel.com/fonts/zilla-slab)
    (friendly with serifs)

6.  [Grand Hotel](https://www.fontsquirrel.com/fonts/grand-hotel) (for a
    fancy script-y feel)

7.  [Montserrat](https://www.fontsquirrel.com/fonts/montserrat)
    (friendly and Sans-like)

8.  [Roboto Slab](https://www.fontsquirrel.com/fonts/roboto-slab)
    (friendly with serifs)

9.  [Molot](https://www.fontsquirrel.com/fonts/molot) (heavier font,
    feels technical)

To use these fonts on your Fedora system, either download the font and
unpack the OTF or TTF file (if required) into your \~/.fonts folder.
...
\$ dnf install open-sans-fonts vernnobile-oswald-fonts
mozilla-zilla-slab-fonts astigmatic-grand-hotel-fonts
julietaula-montserrat-fonts google-roboto-slab-fonts
typetype-molot-fonts

You will need to close and reopen Inkscape, GIMP, or other applications
to take advantage of the new fonts.

If you'd like to post your work somewhere, convert it to a JPG first.
This will typically be a smaller file. Make sure your SVG file is saved
in the fedoramagazine-images/images/ folder as above. Typically these
should be named after the article in some way to make it easier to find
images later. For this example, let's assume your SVG file is named
using-inkscape.svg.
...
Now use the helpful make target we've provided. This creates the correct
sized (1890x800 pixels) JPG automatically, if you provide the same name
as your SVG file:

\$ make using-inkscape.jpg

Now you can post this file, using-inkscape.jpg, online, and then provide
that URL in the comments section of your article's Pagure card for
review.
...
backlog of content is running a little low. Editors take turns serving
as the [Editor of the Week](editor-of-the-week.xml).

Editors follow an [editorial checklist](editorial-checklist.xml) to help
articles meet a quality and content bar, and to ensure they're well
searchable.

Editors hold a [weekly editorial meeting](editorial-meetings.xml) to
discuss and approve content, help guide authors, and figure out ways to
improve the Magazine and further its mission. The best way to become an
editor is to join the weekly meeting, and demonstrate your ability to
contribute consistently.

Editors also [extend access to new
contributors](editorial-add-contributors.xml) so they can help with
writing, art, and sometimes even editing itself.
...
The Editor of the Week will be selected by volunteer basis at each
week's Magazine Editorial Board meeting. Editors who cannot attend the
meeting can volunteer by contacting the current Editor of the Week prior
to the start of the meeting. If the meeting is canceled, the current
Editor of the Week should coordinate selection via the asynchronous
communication channel (i.e. mailing list or Discourse).

If multiple Editors volunteer to be the Editor of the Week, we expect
they can work it out among themselves. However, in general we should
prefer the volunteer who has least recently served as Editor of the Week
in order to fairly distribute the workload.

If no one volunteers, then the current Editor of the Week should request
volunteers via the asynchronous communication channel.

The term of the Editor of the Week is one calendar week (beginning the
Sunday following the meeting where the Editor of the Week was selected
...
list of these articles and proposed publication dates can be found at
this site where XX is the current Fedora Linux release number:
...
Fedora calendar. Meetings are chaired by that week's [Editor of the
Week](editor-of-the-week.xml). If the meeting time changes, be sure to
update the link in this paragraph.

The meeting is powered by
[zodbot](https://fedoraproject.org/wiki/Zodbot). You can use the
following commands the start the meeting, to set topics, record
important information and decisions, etc.
...
Wordpress](https://fedoramagazine.org/admin). On the
...
for a long time without an update, contact the author and ask them if
...
that fact. Review the articles and ether move them to the *to-edit*
column, or in case they need some additional work, ask the author in the
card about it and move it back to *in-progress*.
...
Decide which articles should be published when and set the dates for
each card on the board.

The next step is to vote on the publishing schedule along with other
...
The script consumes data from the individual cards, so please make sure
the date, and the editorrial assignees are set properly.

Find more information about the script in the
[asamalik/magazine-schedule github
repository](https://github.com/asamalik/magazine-schedule).
...
Send the minutes to the forum. That includes the three links zodbot
prints out after the meeting is ended.

Subject: Editorial board meeting recap YYYY-Mmm-DD

The following types of articles generally can route around the weekly
...
they stop being constructive. They may express hostility or include no
valuable ideas --- or simply may be spam.

Therefore, Magazine editors follow a common set of guidelines to approve
...
1.  Ask the writer to login to [Fedora Magazine Pagure
    instance](https://pagure.io/fedora-magazine-newsroom/boards/articles)
    if they have not done so already. This will require that they have a
    Fedora Account System (FAS) login.
...
submit a change, start by forking and/or cloning this repository. This
page does not cover git workflow; consult [git
documentation](https://git-scm.com/doc) for more information, or ask the
Magazine staff for help.
...
Once the theme modifications are complete, you should make a release.
This means that anyone can return to this iteration of the theme in case
...
1.  From the [Kanban
    page](https://pagure.io/fedora-magazine-newsroom/boards/articles),
    select the *New issues* button (upper right corner).

2.  Enter the title of the article in the top text box.

3.  Enter details of the proposal in the bottom text box. *Include a
    link to the proposal discussion*, where appropriate.

4.  If an author has been identified and has logged in to Pagure, select
    the *Assignee* text box in the METADATA column at the right and
    locate their name.

5.  Select *Create Issue* at the bottom of the page.

When the new issue has been created, continue with these steps.

1.  Select the *pencil icon* at the top of the METADATA column at the
    right.

2.  In the *Tags* selection field select *article* and *needs-image*.
...
*to-edit*, drag the cards on the Kanban board, and add a comment to that
effect to inform the author.

Editors should be admins on the board. You must be an Administrator to
add an editor.
...
4.  Type in the FAS Username in the first text field, to search. Note
    that the user must have logged into Pagure prior to this operation
    for the name to be available.

5.  Set the next field (a selection field) to *Admin*.
...
We use real names on the Magazine for authors. This reflects
accountability for posts on a more official Fedora outlet.

Locate the user in the *Users* control panel, and select *Edit*. Switch
the default display of the user name to first and last name.
...
The information in this widget must be changed when a new release
occurs. This section describes the process for making the necessary
changes.
...
meetings](editorial-meetings.xml). Importantly, Zinebot is just an aid.
It is not required to run the meetings. Its main purpose is to fetch
information from various sources to save the meeting chair from having
to jump between several browser tabs or applications and copy-paste
information to or from the meeting chatroom. The meetings can still be
run in the traditional way if Zinebot crashes or otherwise fails to
function.

See [the Fedora Magazine chatbot
repo](https://pagure.io/fedora-magazine-chatbot) for installation
instructions. A good way to test Zinebot outside of meeting times is to
create a private room on [the Fedora Project Matrix
homeserver](https://chat.fedoraproject.org) and invite the *zinebot*
user to the room. You will need to assign the room a [local
address](https://github.com/vector-im/element-web/issues/13077). You can
then configure Zinebot to join that room.
...
The full list of predefined compose keys can be found in the
...
You can customize the single character codes that Zinebot will respond
to by editing the \~/.local/opt/zinebot/commands/hotkeys.pm file.
::::
...
board column and write them out to the chatroom as a summarized list.
Included in the summary for each card will be an indication of whether
or not the author has signed the Fedora Project Contributor Agreement
(FPCA). The status of the author's FPCA signature is retrieved directly
from the Fedora Account System each time a card is retrieved from the
kanban board.
...
When the *to-edit* column is loaded in the current internal card deck,
this command will read the scheduled publication dates from all the
cards in the deck and list them out along with the card titles and
assigned editors as a proposed editing and publication schedule.

As a shorthand for this command, Zinebot will accept a lone unicode
*because* symbol (∵). The call sign should be omitted when using the
shorthand.
...
Discourse forum](https://discussion.fedoraproject.org). The success or
failure of each attempted post is indicated by a respective 🔹 or 🔸 at
the end of each scheduled card. Only articles that are scheduled for the
upcoming week should post successfully. Anything that is scheduled for
an earlier or later date should automatically fail to post.

As a shorthand for this command, Zinebot will accept a lone unicode
*therefore* symbol (∴). The call sign should be omitted when using the
shorthand.
...
This command scans [the Fedora Project QA
calendar](https://calendar.fedoraproject.org/QA) for test day events and
...
This command scans [the Fedora Project
schedule](https://fedorapeople.org/groups/schedule) for scheduled Fedora
Magazine publications such as beta and final release announcements. The
results are then listed in the chatroom.
...
against Pagure. This command can access cards that are not in the
current column. The following card properties are accessible through
this command.
...
target column can be specified. By default, cards are moved to the top
of the target column (row 1).

As a shorthand, you can use the unicode characters ← or → in place of
the column name to signify the previous or next column respectively.
...
from the current day. If the time is not specified, it defaults to the
beginning of the current hour. Dates must be specified as YYYY-MM-DD.
Times must be specified as HH:MM:SS. Times are in Coordinated Universal

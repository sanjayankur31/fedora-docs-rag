- Are you willing to enhance Fedora Project marketing initiatives?

- Have you tuned into any {team_name} meetings?

- Do you know the basics of GitLab?

If you said YES to one or more points above, you are at the right place!
Our team always looks forward to working with folks from diverse
backgrounds.

**Ask us in our [Matrix room]({team_synch_communication}) how to join
the team!**

# How you can contribute {#contribute}

Here are some examples of what constitutes a contribution to the
{team_name}:

- Managing talking points for one or more editions, or parts of Fedora
  (a Fedora Edition, a major feature, etc.).

- Writing a release announcement or contributing a large section to a
  release announcement.

  - Fixing a comma splice is not really a significant contribution.
    Managing a section on the Workstation Edition is.

- Working to create one or more flyers or information sheets for an
  edition, Fedora overall, etc.

- Substantial work helping clean up the Marketing docs.

- Creating materials for Fedora Ambassadors for an event, or talking
  points for an event or events.

- Any observable significant and useful contribution that moves Fedora
  Marketing forward during a release cycle.

- Mentor and bring in new contributors to marketing.

We grant membership to anyone who invests in teaching other people the
ropes, even if that means their impact is not as visible on the finished
product as if they did the work themselves.

A good recommendation for beginners is to familiarize yourself with the
[the tools we use](resources.xml) in order to help with a particular
kind of work.

# Join our meetings {#meetings}

- Meetings will be held once every two weeks. Check the [**Marketing
  Calendar**]({team_meetings}) for updated meeting times.

  - Video Meetings on Jitsi;

  - Recorded and posted on PeerTube

  - Audio and Video is not required to join the meeting (you can always
    use the chat for input)

- Meeting Notes:

  - Will be held in the [Marketing
    HackMD](https://hackmd.io/team/fedoramarketing?nav=overviewFedora)

  - Meeting leader will then post [on
    Discourse]({team_asynch_communication}) for community to provide
    transparency to our community

# Have ideas for social media posts? {#social-media}

- **[Open a ticket on our Gitlab]({team_issue_tracker}/-/issues/new):**

  - Issue name must include Month/Day then title of Social Media post

    - **EX: 10/29 EPEL Campaign Post**

  - The ticket Description:

    - **Purpose of ticket**

    - **Any art ideas for the post**

    - **Potential Caption idea**

Feedback from official {team_name} members must be provided before
accepted to post.

# Want to help with videos? {#videos}

- Check [our standard procedure to process
  livestreams](sops/stream-uploads.xml) to know how you can easily take
  livestreams from our events and turn them into talks to upload to our
  social media profiles;

- Check the [Podcast team GitLab
  repo](https://gitlab.com/fedora/podcast/-/issues) if you'd like to
  help with the Fedora Podcast, as they have a tight relation to our
  team;

# Fedora social media accounts {#_fedora_social_media_accounts}

Overview of all known social media accounts for the Fedora Project.
2023-11-19

Social media connects Fedora developers, users, friends, and enthusiasts
around the world. This is also one strategy to share Fedora
announcements, community news, and other updates. All known accounts are
documented below, curated by a best-effort volunteering community.

## Official accounts {#official}

Maintained by Marketing and/or [Social
Media](https://accounts.fedoraproject.org/group/fedora-socialmedia/)
Teams. Anyone with access to Official Accounts should be members of the
[fedora-socialmedia](https://accounts.fedoraproject.org/group/fedora-socialmedia/)
FAS group. If you believe you should be in the group, contact a [group
sponsor](https://accounts.fedoraproject.org/group/fedora-socialmedia/).

:::: note
::: title
:::

Official accounts are designated official due to their connection to the
Fedora Trademark.
::::

### Mastodon {#official-mastodon}

- **Page name**: Fedora Project

- **Links**:

  - [Mastodon](https://joinmastodon.org/)

  - [fosstodon.org/@fedora](https://fosstodon.org/@fedora)

- **Administrators**:

  - [Joseph Gayoso]({FWIKI}/User:Joseph)

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

  - [Nick Bebout]({FWIKI}/User:Nb)

- **Clients**: open source decentralized servers, web-based, clients
  available on mobile (Android, iOS)

### Facebook {#official-facebook}

- **Page name**: The Fedora Project

- **Links**:

  - [Facebook](https://www.facebook.com)

  - [facebook.com/TheFedoraProject](https://www.facebook.com/TheFedoraProject)

- **Administrators**:

  - **Administrators (full access)**:

    - [Brian Proffitt]({FWIKI}/User:Bproffit)

    - [Paul W. Frields]({FWIKI}/User:Pfrields)

    - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

    - [Joe Brockmeier]({FWIKI}/User:Jzb)

    - [Christoph Wickert]({FWIKI}/User:Cwickert)

    - [Robyn Bergeron]({FWIKI}/User:Rbergero)

    - [Jiří Eischmann]({FWIKI}/User:Eischmann)

    - [Tom Callaway]({FWIKI}/User:Spot)

    - [Ruth Suehle]({FWIKI}/User:Rsuehle)

  - **Editors**:

    - [Remy DeCausemaker]({FWIKI}/User:Decause)

    - [Diaa Radwan]({FWIKI}/User:Diaa)

- **Clients**: proprietary server, web-based, messaging supported in
  `bitlbee`, `pidgin`

### X/Twitter {#official-twitter}

- **Page name**: \@fedora

- **Links**:

  - [X/Twitter](https://twitter.com/)

  - [twitter.com/fedora](https://twitter.com/fedora)

- **Administrators**:

  - [Brian Proffitt]({FWIKI}/User:Bproffit)

  - [Paul W. Frields]({FWIKI}/User:Pfrields)

  - [Ruth Suehle]({FWIKI}/User:Rsuehle)

  - [Joe Brockmeier]({FWIKI}/User:Jzb)

  - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

  - [Matthew Miller]({FWIKI}/User:Mattdm)

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

  - [ Jona Azizaj]({FWIKI}/User:Jonatoni)

  - [Marie Nordin]({FWIKI}/User:Riecatnor)

  - [Vipul Siddharth]({FWIKI}/User:siddharthvipul1)

- **Clients**: proprietary server, web-based, clients available
  (`cawbird`)

### YouTube {#official-youtube}

- **Page name**: Fedora Project

- **Links**:

  - [YouTube](https://www.youtube.com)

  - [youtube.com/fedora](https://www.youtube.com/fedora)

- **Administrators**:

  - **Primary owner**: *Fedora Project (social media mgmt.)* (special
    Google account managed by the [Fedora Community
    Architect](council::fca.xml) specifically for YouTube)

  - **Owners**:

    - [Justin W. Flory]({FWIKI}/User:Jflory7)

    - [Brian Proffitt]({FWIKI}/User:Bproffit)

  - **Managers**:

    - [Adrian Edwards]({FWIKI}/User:Moralcode)

    - [Akashdeep Dhar]({FWIKI}/User:T0xic0der)

    - [Alberto Rodríguez Sánchez]({FWIKI}/User:Bt0dotninja)

    - [Ben Cotton]({FWIKI}/User:Bcotton)

    - [Daimar Stein]({FWIKI}/User:Steiner)

    - [Eduard Lucena]({FWIKI}/User:X3mboy)

    - [Eric Hendricks]({FWIKI}/User:Itguyeric)

    - [Jiří Eischmann]({FWIKI}/User:Eischmann)

    - [Kevin Fenzi]({FWIKI}/User:Kevin)

    - [Marie Nordin]({FWIKI}/User:Riecatnor)

    - [Mark Terranova]({FWIKI}/User:Markdude)

    - [Paul W. Frields]({FWIKI}/User:Pfrields)

    - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

    - [Tom Callaway]({FWIKI}/User:Spot)

- **Clients**: proprietary server, web-based, supported in
  `video-downloader`

### Instagram {#official-instagram}

- Page name: \@thefedoraproject

- **Links**:

  - [Instagram](https://www.instagram.com)

  - [instagram.com/thefedoraproject](https://www.instagram.com/thefedoraproject/)

- **Administrators**:

  - [Daimar Stein]({FWIKI}/User:Steiner)

  - [Emma Kidney]({FWIKI}/User:Ekidney)

  - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

  - [Jona Azizaj]({FWIKI}/User:Jonatoni)

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

- **Clients**:

  - proprietary server, has web version

### Matrix/Element {#official-matrix}

- **Page name**: `#fedora-space:fedoraproject.org`

- **Links**:

  - link:https://chat.fedoraproject.org/*/room/#fedora-space:fedoraproject.org\[chat.fedoraproject.org/*/room/#fedora-space:fedoraproject.org\]

- **Administrators**:

  - [Kevin Fenzi]({FWIKI}/User:Kevin)

  - [Nick Bebout]({FWIKI}/User:Nb)

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

- **Clients**: open source server, has web version, open source client
  available as flatpak in
  [flathub](https://flathub.org/apps/details/im.riot.Riot)

## Secondary accounts {#secondary}

Secondary accounts are accounts created officially within the project,
but they are not the primary method for distributing news or
information. Often, these accounts post automatically or pull from other
feeds to relay content.

### Twitter (@fedoracommunity) {#secondary-twitter}

[Ryan Lerch]({FWIKI}/User:Ryanlerch) has/had a script to automatically
tweet links for new [Community
Blog](https://docs.fedoraproject.org/en-US/commops/community-blog/)
articles.

- **Page name**: \@fedoracommunity

- **Links**:

  - [Twitter](https://twitter.com/)

  - [twitter.com/fedoracommunity](https://twitter.com/fedoracommunity)

- **Administrators**:

  - [Brian Exelbierd]({FWIKI}/User:Bex)

  - [Jona Azizaj]({FWIKI}/User:Jonatoni)

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

  - [Marie Nordin]({FWIKI}/User:Riecatnor)

  - [Paul W. Frields]({FWIKI}/User:Pfrields)

  - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

### Reddit (/r/Fedora) {#secondary-reddit}

- **Page name**: /r/Fedora

- **Links**:

  - [Reddit](https://www.reddit.com/)

  - [reddit.com/r/Fedora/](https://www.reddit.com/r/Fedora/)

- **Administrators**:

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

  - [Eric Gustavsson]({FWIKI}/User:SpyTec)

- **Clients**: proprietary server, web-based

### Discord {#secondary-discord}

- **Page name**: Fedora Linux

- **Links**:

  - [Discord](https://discordapp.com)

  - [discord.gg/fedora](https://discord.gg/fedora)

- **Administrators**:

  - *Owner*: [Radka (Rhea) Gustavsson]({FWIKI}/User:Rhea)

  - *Moderators*:

    - [Eric Eisenhart]({FWIKI}/User:Ffreiheit)

    - DokaDoka

    - dvs

    - refi64

    - Sebastiaan

    - -Kass

- **Clients**: proprietary server, has web version, open source clients
  are forbidden by license terms

### Telegram {#secondary-telegram}

- **Page name**: \@fedoranews

- **Links**:

  - [Telegram](https://telegram.org/)

  - [\@fedoranews](https://t.me/fedoranews)

- **Administrators**:

  - [Justin W. Flory]({FWIKI}/User:Jflory7)

  - [Jiří Eischmann]({FWIKI}/User:Eischmann)

  - [Ryan Lerch]({FWIKI}/User:Ryanlerch)

  - [Marie Nordin]({FWIKI}/User:Riecatnor)

  - [Sumantro Mukherjee]({FWIKI}/User:Sumantrom)

  - [Eduard Lucena]({FWIKI}/User:x3mboy)

  - [Vipul Siddharth]({FWIKI}/User:siddharthvipul1)

- **Clients**: proprietary server, has web version, open source client
  available in RPMFusion

## Community-managed accounts {#community}

Fedora Community-Managed Accounts are related to the Fedora Project, but
not officially maintained by the Fedora Marketing Team or Social Media
administrators. Often, these accounts are specific to a region or
country. These pages are typically run by an individual contributor or a
group of contributors. However, these accounts are **not** officially
endorsed by the Fedora Project or Red Hat Inc. They are listed here to
promote easier discoverability of places where Fedora contributors and
users gather online.

### Facebook {#community-facebook}

+----------------------+----------------------+-----------------------+
| Page name            | Links                | Administrator(s)      |
+======================+======================+=======================+
| Fedora Albania       | [facebook.com/fedor  | [Jona                 |
|                      | asq](https://www.fac | Azizaj]({             |
|                      | ebook.com/fedorasq/) | FWIKI}/User:jonatoni) |
|                      |                      | (via [Open Labs       |
|                      |                      | Albania](h            |
|                      |                      | ttps://openlabs.cc/)) |
+----------------------+----------------------+-----------------------+
| Fedora Bangladesh    | [facebook            | ???                   |
| (group)              | .com/groups/fedoraba |                       |
|                      | ngladesh](https://ww |                       |
|                      | w.facebook.com/group |                       |
|                      | s/fedorabangladesh/) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Project -     | [facebook.com/fe     | [Yan Naing            |
| Local Community      | dorammcommunity](htt | Myint]({FWIKI         |
| Myanmar              | ps://www.facebook.co | }/User:Orcakrilozona) |
|                      | m/fedorammcommunity) |                       |
+----------------------+----------------------+-----------------------+
| Projeto Fedora       | [facebook.com/       | [Daniel               |
| Brasil               | fedorabr](https://fa | Bruno](               |
|                      | cebook.com/fedorabr) | {FWIKI}/User:Dbruno), |
|                      |                      | [Wolnei]              |
|                      |                      | ({FWIKI}/User:Wolnei) |
+----------------------+----------------------+-----------------------+
| Fedora Ni            | [facebook.co         | [William              |
|                      | m/FedoraNicaragua](h | Moreno]({FWIKI}/      |
|                      | ttps://www.facebook. | User:Williamjmorenor) |
|                      | com/FedoraNicaragua) | (owner), [Eduardo     |
|                      |                      | Mayorga]({FWIK        |
|                      |                      | I}/User:Mayorgalinux) |
+----------------------+----------------------+-----------------------+
| Fedora Pakistan      | [facebook.           | [Nasir                |
|                      | com/FedoraPakistan]( | Hussain](             |
|                      | https://www.facebook | {FWIKI}/User:Nasirhm) |
|                      | .com/FedoraPakistan) |                       |
+----------------------+----------------------+-----------------------+

### Twitter {#community-twitter}

+----------------------+----------------------+-----------------------+
| Account name         | Links                | Administrator(s)      |
+======================+======================+=======================+
| Fedora Albania       | [twitter.com         | [Jona                 |
|                      | /fedorasq](https://t | Azizaj]({             |
|                      | witter.com/fedorasq) | FWIKI}/User:jonatoni) |
|                      |                      | (via [Open Labs       |
|                      |                      | Albania](h            |
|                      |                      | ttps://openlabs.cc/)) |
+----------------------+----------------------+-----------------------+
| RussianFedora        | [t                   | [Inna                 |
|                      | witter.com/RussianFe | Kabanova](            |
|                      | dora](https://twitte | {FWIKI}/User:Mamasun) |
|                      | r.com/RussianFedora) |                       |
+----------------------+----------------------+-----------------------+
| Fedora in Greece     | [twitter.com/f       | ???                   |
|                      | edora_gr](https://tw |                       |
|                      | itter.com/fedora_gr) |                       |
+----------------------+----------------------+-----------------------+
| Fedora™ Indonesia    | [twitt               | [Jurankdankkal](      |
|                      | er.com/FedoraIndones | {FWIKI}/User:Mamasun) |
|                      | ia](https://twitter. |                       |
|                      | com/FedoraIndonesia) |                       |
+----------------------+----------------------+-----------------------+
| ProjetoFedora        | [t                   | [Daniel               |
|                      | witter.com/projetofe | Bruno](               |
|                      | dora](https://twitte | {FWIKI}/User:Dbruno), |
|                      | r.com/projetofedora) | [Wolnei]              |
|                      |                      | ({FWIKI}/User:Wolnei) |
+----------------------+----------------------+-----------------------+
| Fedora Nicaragua     | [twitter.com         | [William              |
|                      | /FedoraNi](https://t | Moreno]({FWIKI}/      |
|                      | witter.com/FedoraNi) | User:Williamjmorenor) |
|                      |                      | (owner), [Eduardo     |
|                      |                      | Mayorga]({FWIK        |
|                      |                      | I}/User:Mayorgalinux) |
+----------------------+----------------------+-----------------------+
| NeuroFedora          | [twitter.com/neuro   | [Ankur                |
|                      | fedora](https://twit | Sinha]({FW            |
|                      | ter.com/neurofedora) | IKI}/User:Ankursinha) |
|                      |                      | and the [NeuroFedora  |
|                      |                      | SIG]({FWIK            |
|                      |                      | I}/SIGs/NeuroFedora). |
|                      |                      | See the               |
|                      |                      | [SOP](                |
|                      |                      | https://pagure.io/neu |
|                      |                      | ro-sig/NeuroFedora/bl |
|                      |                      | ob/master/f/Outreach/ |
|                      |                      | Template-tweets.rst). |
+----------------------+----------------------+-----------------------+
| Fedora Haskell       | [t                   | [Jens                 |
|                      | witter.com/fedorahas | Petersen]({           |
|                      | kell](https://twitte | FWIKI}/User:Petersen) |
|                      | r.com/fedorahaskell) | and the [Haskell      |
|                      |                      | SIG](                 |
|                      |                      | {FWIKI}/SIGs/Haskell) |
+----------------------+----------------------+-----------------------+
| Fedora i18n          | [twitter.com/Fed     | [Jens                 |
|                      | oraI18n](https://twi | Petersen]({           |
|                      | tter.com/FedoraI18n) | FWIKI}/User:Petersen) |
|                      |                      | and I18N              |
+----------------------+----------------------+-----------------------+
| Fedora Czech         | [twitter.com         | ???                   |
|                      | /FedoraCZ](https://t |                       |
|                      | witter.com/FedoraCZ) |                       |
+----------------------+----------------------+-----------------------+
| Fedora CoreOS        | [twitter.com/FedoraC | ???                   |
|                      | oreOS](https://twitt |                       |
|                      | er.com/FedoraCoreOS) |                       |
+----------------------+----------------------+-----------------------+
| Fedora IoT           | [twitter.com/F       | ???                   |
|                      | edoraIoT](https://tw |                       |
|                      | itter.com/FedoraIoT) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Silverblue    | [twi                 | ???                   |
|                      | tter.com/teamsilverb |                       |
|                      | lue](https://twitter |                       |
|                      | .com/teamsilverblue) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Türkiye       | [twitter.com/fedorat | ???                   |
|                      | urkey](https://twitt |                       |
|                      | er.com/fedoraturkey) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Mexico        | [twitter.com/fedoram | ???                   |
|                      | exico](https://twitt |                       |
|                      | er.com/fedoramexico) |                       |
+----------------------+----------------------+-----------------------+
| Fedora India         | [twitter.com/fedor   | [Vipul                |
|                      | aindia](https://twit | Siddharth]({FWIKI}/   |
|                      | ter.com/fedoraindia) | User:siddharthvipul1) |
+----------------------+----------------------+-----------------------+
| Fedora Bot           | [twitt               | ???                   |
|                      | er.com/FedoraBot60_M |                       |
|                      | PH](https://twitter. |                       |
|                      | com/FedoraBot60_MPH) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Security Lab  | [twitter.com         | ???                   |
|                      | /FedoraSL](https://t |                       |
|                      | witter.com/FedoraSL) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Community     | [t                   | [Justin W.            |
| Space                | witter.com/i/communi | Flory](               |
|                      | ties/151109535749686 | {FWIKI}/User:Jflory7) |
|                      | 8869](https://twitte |                       |
|                      | r.com/i/communities/ |                       |
|                      | 1511095357496868869) |                       |
+----------------------+----------------------+-----------------------+

### Other accounts {#community-other}

+-----------------+-----------------+-----------------+-----------------+
| Network         | Page name       | Links           | A               |
|                 |                 |                 | dministrator(s) |
+=================+=================+=================+=================+
| [F              | FEDORA          | [flick          | ???             |
| lickr](https:// | GNU/linux       | r.com/groups/fe |                 |
| www.flickr.com) |                 | dora/](https:// |                 |
|                 |                 | www.flickr.com/ |                 |
|                 |                 | groups/fedora/) |                 |
+-----------------+-----------------+-----------------+-----------------+
| [Fedora-IT      | Fedora Italy    | [foru           | [ro             |
| ](http://www.fe |                 | m.fedoraonline. | byduck]({FWIKI} |
| doraonline.it/) |                 | it](http://foru | /User:Robyduck) |
|                 |                 | m.fedoraonline. |                 |
|                 |                 | it/search.php?a |                 |
|                 |                 | ction=show_all) |                 |
+-----------------+-----------------+-----------------+-----------------+
| [V              | Russian Fedora  | [vk.c           | [Inna           |
| kontakte](http: |                 | om/club10679202 | K               |
| //vkontakte.ru) |                 | ](https://vk.co | abanova]({FWIKI |
|                 |                 | m/club10679202) | }/User:Mamasun) |
+-----------------+-----------------+-----------------+-----------------+

## Retired accounts {#retired}

These accounts, for one reason or another, are retired, deleted, or no
longer maintained.

+-----------------+-----------------+-----------------+-----------------+
| Network         | Page name       | Links           | A               |
|                 |                 |                 | dministrator(s) |
+=================+=================+=================+=================+
| [d              | <fe             | [di             | [Brian          |
| iaspora\*](http | dora@diasp.org> | asp.org/u/fedor | Pr              |
| s://diasp.org/) |                 | a](https://dias | offitt]({FWIKI} |
|                 |                 | p.org/u/fedora) | /User:Bproffit) |
+-----------------+-----------------+-----------------+-----------------+
| [Identica](htt  | Fedora Project  | [identi.ca/fed  | [Paul W.        |
| p://identi.ca/) |                 | ora](https://id | F               |
|                 |                 | enti.ca/fedora) | rields]({FWIKI} |
|                 |                 |                 | /User:Pfrields) |
|                 |                 |                 | (owner), [Robyn |
|                 |                 |                 | Ber             |
|                 |                 |                 | geron]({FWIKI}/ |
|                 |                 |                 | User:Rbergero), |
|                 |                 |                 | Matias Kreder,  |
|                 |                 |                 | [Jonathan       |
|                 |                 |                 | Nalley]({FWIKI} |
|                 |                 |                 | /User:jnalley), |
|                 |                 |                 | [Jared          |
|                 |                 |                 | Smith]({FWIK    |
|                 |                 |                 | I}/User:Jsmith) |
+-----------------+-----------------+-----------------+-----------------+
| [Linke          | Fedora          | [linke          | [Pascal         |
| dIn](https://ww |                 | din.com/e/gis/4 | Ca              |
| w.linkedin.com) |                 | 9737/](https:// | larco]({FWIKI}/ |
|                 |                 | www.linkedin.co | User:pcalarco), |
|                 |                 | m/e/gis/49737/) | [Francesco      |
|                 |                 |                 | Cri             |
|                 |                 |                 | ppa]({FWIKI}/Fr |
|                 |                 |                 | ancescoCrippa), |
|                 |                 |                 | [Jon            |
|                 |                 |                 | S               |
|                 |                 |                 | tanley]({FWIKI} |
|                 |                 |                 | /User:Jstanley) |
+-----------------+-----------------+-----------------+-----------------+
| [               | \@fedora_planet | [t              | Automatically   |
| Twitter](https: |                 | witter.com/fedo | tweets out new  |
| //twitter.com/) |                 | ra_planet](http | [Fedora         |
|                 |                 | s://twitter.com | Planet](        |
|                 |                 | /fedora_planet) | {FWIKI}/Planet) |
|                 |                 |                 | articles        |
+-----------------+-----------------+-----------------+-----------------+

### Google+ {#retired-googleplus}

The Google+ service shut down [on 2019 April
2](https://support.google.com/plus/answer/9217723).

+----------------------+----------------------+-----------------------+
| Account name         | Links                | Administrator(s)      |
+======================+======================+=======================+
| Fedora Project       | [plus.google.com/+   | **Owner**: [Matthew   |
|                      | Fedora](https://plus | Williams]({FWIKI}/U   |
|                      | .google.com/+Fedora) | ser:Lorddrachenblut)\ |
|                      |                      | **Managers**: [Brian  |
|                      |                      | Exelbier              |
|                      |                      | d]({FWIKI}/User:Bex), |
|                      |                      | [Brian                |
|                      |                      | Proffitt]({F          |
|                      |                      | WIKI}/User:Bproffit), |
|                      |                      | Caius CHANCE,         |
|                      |                      | [Christoph            |
|                      |                      | Wickert]({F           |
|                      |                      | WIKI}/User:Cwickert), |
|                      |                      | Izhar Firdaus         |
|                      |                      | (KageSenshi), [Jiří   |
|                      |                      | Eischmann]({FW        |
|                      |                      | IKI}/User:Eischmann), |
|                      |                      | [Joe                  |
|                      |                      | Brockmeie             |
|                      |                      | r]({FWIKI}/User:Jzb), |
|                      |                      | [Justin W.            |
|                      |                      | Flory]({              |
|                      |                      | FWIKI}/User:Jflory7), |
|                      |                      | [Kevin                |
|                      |                      | Fenzi]                |
|                      |                      | ({FWIKI}/User:Kevin), |
|                      |                      | [Mark                 |
|                      |                      | Terranova]({F         |
|                      |                      | WIKI}/User:Markdude), |
|                      |                      | [Paul W.              |
|                      |                      | Frields]({F           |
|                      |                      | WIKI}/User:Pfrields), |
|                      |                      | [Remy                 |
|                      |                      | DeCausemaker]({       |
|                      |                      | FWIKI}/User:Decause), |
|                      |                      | [Ruth                 |
|                      |                      | Suehle]({             |
|                      |                      | FWIKI}/User:Rsuehle), |
|                      |                      | [Ryan                 |
|                      |                      | Lerch]({FW            |
|                      |                      | IKI}/User:Ryanlerch), |
|                      |                      | [Tom                  |
|                      |                      | Callaway              |
|                      |                      | ]({FWIKI}/User:Spot), |
|                      |                      | [Matthew              |
|                      |                      | Miller](              |
|                      |                      | {FWIKI}/User:Mattdm)\ |
|                      |                      | **Communications      |
|                      |                      | manager**: [Charles   |
|                      |                      | Profitt]({            |
|                      |                      | FWIKI}/User:Cprofitt) |
+----------------------+----------------------+-----------------------+
| Fedora Albania       | [plus.goog           | [g                    |
|                      | le.com/communities/1 | 1m]({FWIKI}/User:G1m) |
|                      | 05327757478147133758 |                       |
|                      | ](https://plus.googl |                       |
|                      | e.com/communities/10 |                       |
|                      | 5327757478147133758) |                       |
+----------------------+----------------------+-----------------------+
| Proyecto Fedora      | [plus.goog           | [gomix                |
|                      | le.com/communities/1 | ]({FWIKI}/User:Gomix) |
|                      | 03027051591113621787 |                       |
|                      | ](https://plus.googl |                       |
|                      | e.com/communities/10 |                       |
|                      | 3027051591113621787) |                       |
+----------------------+----------------------+-----------------------+
| Fedora Nicaragua     | [plus.go             | [William              |
|                      | ogle.com/+FedoraOrgN | Moreno]({FWIKI}/      |
|                      | i](https://plus.goog | User:Williamjmorenor) |
|                      | le.com/+FedoraOrgNi) | (owner), [Eduardo     |
|                      |                      | Mayorga]({FWIK        |
|                      |                      | I}/User:Mayorgalinux) |
+----------------------+----------------------+-----------------------+
| Fedora Portugal      | [plus.goog           | [Pedro                |
|                      | le.com/communities/1 | Andrade]              |
|                      | 09959996284119569153 | ({FWIKI}/User:Cifose) |
|                      | ](https://plus.googl | (owner), [Nuno        |
|                      | e.com/communities/10 | Rodrigues]            |
|                      | 9959996284119569153) | ({FWIKI}/User:Nunor5) |
+----------------------+----------------------+-----------------------+

## Account security {#security}

Most official social media accounts are managed by each administrator's
personal account on that network, like for Facebook or Google+. In cases
like LinkedIn, access is granted to individual accounts. However, single
passwords still exist for most of these accounts. The passwords are
securely stored within Fedora's infrastructure, along with private
assets used by the [Infrastructure team](Infrastructure). As of October
2016, User:Puiterwijk\[Patrick Uiterwijk\] manages password storage and
security.

## Historical context {#historical}

- Social media was briefly discussed at the [Marketing FAD
  2010]({FWIKI}/Marketing_FAD_2010). [complete logs are available (start
  at
  14:10:39)](http://meetbot.fedoraproject.org/fedora-fad/2010-03-15/fedora-fad.2010-03-15-13.15.log.html).
  For a shorter recap, see this [video of Neville Cross; Ryan Rix; and
  Paul Frields explaining the
  discussion](http://www.youtube.com/watch?v=oh9Yi_2jI_g) and [a diagram
  of the whiteboard (photo by Henrik
  Heigl)](http://www.flickr.com/photos/wonderer/4462227596/).

- [Status and microblogging SOP]({FWIKI}/Status_and_microblogging_SOP)

# Fedora Social Media Contribution Model {#_fedora_social_media_contribution_model}

Learn more about the Fedora Marketing Team social media model and
discover how you can contribute.

The Fedora Marketing Team operates a three-tiered model for social media
contributions:

1.  [**Social Media Manager**](#social-media-manager)

2.  [**Content Contributor**](#content-contributor)

3.  [**Community Contributor**](#community-contributor)

Each role focuses on different types of contributions, such as posting
content, introducing new ideas for our social media, and sharing updates
about other parts of the Fedora Project to keep our users informed.
Members of each role have different access levels to our social media
accounts. For instance, Social Media Managers have higher level access
to our social media account than those in the other roles. Read on for
details of each role, types of contributions made within them and how
you can get involved.

## Social Media Manager

Social Media Managers are granted direct access to our social media
accounts and are categorized into two specific functions: [**Account
Lead**](#account-lead) and [**Account
Contributor**](#account-contributor). These functions have varying
levels of contribution and responsibility.

### Account Lead

An Account Lead acts as a bridge, independently cross-posting content
from one Fedora account to another. This involves following Fedora on
other platforms, staying up-to-date on their activities, and copying
content to your assigned account. You do not need to source or create
new content to share; your focus is on sharing existing content.

The Account Lead role has a more advanced and involved version known as
the Curator. Curators take full ownership of a social media account,
independently sourcing content, crafting posts, and actively engaging
with the Fedora and Linux communities. This demands a higher level of
responsibility, proactive engagement, and a deep understanding of
Fedora, its \"products,\" stakeholders, and community dynamics.

### Account Contributor

As an Account Contributor, your main responsibility is to post social
media content provided by others. For instance, you will get a ping by
someone with what to post. You do not need to come up with content on
your own, but focus on executing the posting requests from others. A key
aspect of this role is being readily available to receive content
submissions from others.

### Ways to Contribute {#ways}

- Social media management

- Engaging with our users on social media

- Posting content on our social media accounts

- Checking and responding to comments, tags, and direct messages

- Working with individuals in the other roles to schedule and post
  content

### Our Social Media Management Team {#social-media-team}

+-----------------------------------+-----------------------------------+
| Social account                    | Social Media Manager              |
+===================================+===================================+
| Twitter                           | \@jwflory                         |
+-----------------------------------+-----------------------------------+
| LinkedIn                          | \@jwflory                         |
+-----------------------------------+-----------------------------------+
| Mastodon                          | \@josephgayoso                    |
+-----------------------------------+-----------------------------------+
| Instagram                         | \@ekidney                         |
+-----------------------------------+-----------------------------------+
| Youtube                           | \@ekidney                         |
+-----------------------------------+-----------------------------------+

<figure>
<img src="stream-uploads/social-media-contribution-model.png"
alt="A visual aid titled &quot;Social Media Contribution Model&quot;. There are four rings of different sizes stacked on top of each other" />
<figcaption>social media contribution model</figcaption>
</figure>

## Content Contributor

Content Contributors actively contribute content ideas for social media
posts. They assist with issues on GitLab that the marketing team needs
help with. They do not have direct access to our social media accounts
but collaborate with social media managers to share content. They help
in managing social media campaigns, promoting Fedora's initiatives and
events. These contributors curate content for use by the Social Media
Managers.

### Ways to Contribute {#ways-to}

- Come up with interesting ideas to post about. Write a post on that
  topic, then follow the submission process below. Make sure the content
  is engaging and aligns with Fedora's messaging and voice.

- Work with the [Marketing Team](need-help.xml) to coordinate and manage
  social media campaigns.

- Curate content for use by the Lead.

Follow the steps below to submit your content idea to our Social Media
Managers.

### How to submit your post to the Social Media Managers {#sumbit-post}

- Open an issue on GitLab for your idea/content in the Fedora [Marketing
  Planning](https://gitlab.com/fedora/marketing/marketing-planning/-/issues)
  repo.

- Choose ****social-media-template**** and follow the prompt to submit
  your idea.

- A Social Media Manager will review your submission, provide feedback,
  and approve it for posting.

## Community Contributor

Community Contributors are people whose primary role at Fedora is not
marketing, communications, or promotion. They come to the Marketing Team
for assistance on promoting their content on social media, such as:
social media coverage for specific events, articles, or announcements.

### Ways to Contribute {#ways-to-contribute}

- Reach out to the Marketing Team with topics, ideas, or requests for
  social media coverage. The easiest way is to join our [Matrix
  room](https://matrix.to/#/#marketing:fedoraproject.org) and simply
  mention what assistance you need. Usually we can accommodate these
  requests without having to create a ticket on GitLab. However, for
  larger or more complex request, you can create a ticket on GitLab. But
  before then, the team prefers that you ask for help first, and if the
  request is large, the team will create a ticket if necessary.

- Inform the marketing team about upcoming events where Fedora will have
  a presence, requesting promotional coverage.

## Changelog

For added transparency about when changes are made to this document, a
changelog is maintained below.

+----------------------+----------------------+-----------------------+
| Version \#           | Authors              | Description           |
+======================+======================+=======================+
| 1.0                  | Roseline Bassey;     | Initial release.      |
|                      | Joseph Gayoso;       | Copyediting led by    |
|                      | Justin W. Flory      | Bassey.               |
+----------------------+----------------------+-----------------------+

: Social Media Contribution Model changelog

# Marketing Resources {#_marketing_resources}

These resources will help you take Fedora to the people.

## Where to find the Fedora Marketing Team {#contact}

- [Our room on Matrix]({team_synch_communication})

- [Our tag on Fedora Discussion]({team_asynch_communication})

- [Our planning discussions on GitLab]({team_issue_tracker}/-/issues)

The best way to find and contact the Marketing team is on Matrix/Element
or by opening a ticket on GitLab.

## The social media profiles we currently manage {#social-media}

- [Mastodon](https://fosstodon.org/@fedora)

- [Instagram](https://www.instagram.com/thefedoraproject/)

- [Twitter](https://twitter.com/fedora)

- [LinkedIn](https://www.linkedin.com/company/fedora-project/)

- [YouTube](https://www.youtube.com/@fedora/)

Don't see your favorite social media platform here? **Help us spread the
word by joining as a social media manager or helping create content for
it!**

## The tools we use in our work {#tools}

- We use [HackMD](https://hackmd.io/) for working in collaboration in
  our notes;

- We use both [Penpot](https://design.penpot.app) and
  [Inkscape](https://inkscape.org/) for our graphic design work. They
  are the same tools the Fedora Design Team uses, so you can also help
  the Design Team out by learning to use our tools.

# Introducing Fedora Ready {#_introducing_fedora_ready}

Fedora Ready is a program by the Fedora Project to support vendors who
ship or support Fedora Linux on their hardware. 2024-07-31

:::: note
::: title
:::

This article is draft content and is not yet finalized. This notice will
be removed once the Fedora Ready program is officially launched.
::::

The Fedora Ready program supports vendors who meet our criteria in their
support of Fedora Linux on their devices. Also, we want to simplify the
process for our users to find and purchase these supported devices. Our
[central hub](ready/list.xml) lists all Fedora Ready devices so people
can easily locate where to buy them.

## What Fedora Ready is not {#fedora-ready-is-not}

- Fedora Ready is NOT a certification or endorsement program.

- Fedora Ready is not a formal partnership program nor are we in legal
  partnership with vendors listed on our compatibility list. Vendors
  oversee the lifecycle of their devices and offer what makes a good
  experience for their customers.

## Criteria for Vendor Listing {#criteria}

Vendors are listed under Fedora Ready if:

- A vendor is selling a computer with Fedora Linux available
  pre-installed:

  - We have reputable contact with decision-makers or points of contact
    within your team.

  - The computer is available for purchase online.

  - If the product is in the pipeline and not ready yet, then we should
    be in contact in the lead up to the product's launch to tie our
    promotional efforts to that moment.

- A vendor is officially supporting Fedora Linux as an operating system
  for their computer:

  - We have reputable contact with decision-makers or points of contact
    within your team.

  - You need to publicly share which devices have the support for Fedora
    Linux online.

  - If the support is something that is planned, then we should be in
    contact with your team to tie promotion to the product's
    announcement.

- A vendor offers official Fedora Linux support without preinstallation.

- They agree to ship a Fedora Linux Edition (such as [Fedora KDE Plasma
  Desktop](https://fedoraproject.org/kde/) or [Fedora
  Workstation](https://fedoraproject.org/workstation/)) unmodified when
  using the Fedora trademark and are in compliance with all Fedora
  trademark guidelines.

:::: note
::: title
:::

The criteria outlined here are subject to change as the program evolves.
We expect to iterate and refine these criteria based on how vendors
support Fedora Linux and as the program develops over time.
::::

## Criteria for Delisting Vendor {#delisting}

Fedora Ready vendors are hardware companies that meet our criteria in
their support of Fedora Linux on their hardware. To keep our program
running smoothly, we occasionally re-evaluate vendor offerings for
compliance. If a vendor's support of Fedora Linux on their device no
longer align with our criteria, we may need to remove them from the
list. This helps us maintain the program's value.

### A Vendor will be delisted from the program if: {#delisting-criteria}

- A vendor decides to stop supporting Fedora Linux on their devices
  after confirmation.

- A vendor ships a modified Fedora Linux when using the Fedora trademark
  or in a manner that differs from what we offer directly to our users.

- A vendor is non-compliance with all Fedora trademark guidelines.

- A vendor's actions conflict with our values or become detrimental to
  the program's integrity. In such cases, we reserve the right to take
  necessary actions, including removing the vendor and /or product from
  the program, to protect the integrity of the program and our
  trademark.

### Delisting Process

**Initial Communication**: If a vendor does not meet our criteria, we
will notify the vendor about the possibility of being delisted. We will
specify the issues observed and the criteria they have failed to meet.

**Vendor Response Period**: The vendor will have a specified period
(e.g., 30 days) to respond to the notification and implement the
necessary corrective actions to comply with Fedora Ready standards.

**Evaluation of Response**: After the response period, we will
re-evaluate the vendor's status. If the vendor has addressed the issues
satisfactorily, we will continue their listing with a probationary
review scheduled. If the vendor has failed to comply or the response is
unsatisfactory, we will proceed to the next step.

**Final Decision and Notification**: We will make a final decision on
delisting the vendor. If delisting is decided, we will notify the vendor
of the decision and the reasons for it.

**Update Listing**: We will update the [Fedora Ready compatibility
list](ready/list.xml) accordingly.

## Understanding our Limitations {#limitations}

Our program is run by a dedicated team of volunteers, who are not
full-time employees. This means that we do not have full-time
availability or dedicated company resources, but our commitment to
supporting vendors remains unwavering.

### What This Means for Vendors {#vendors}

- **Availability:** Our team members volunteer their time alongside
  their personal and professional commitments. Because of this, our
  response times may be a bit longer than what you might expect from a
  full-time corporate team.

- **Resources:** We do not have specific resources allocated exclusively
  for this program. However, we are here to provide feedback and
  guidance in various forms. Ultimately, it is up to the vendors to
  implement and support Fedora Linux on their devices.

### How We've Supported Vendors in the Past {#past}

Despite these constraints, we have successfully provided support to
vendors in various ways. Here are a few examples of what we have helped
vendors with in the past:

- Two product launches for Slimbook which has included social media
  campaigns, distribution to media outlets, co-writing of the product's
  announcement, and an episode on the [Fedora
  Podcast](https://fedoraproject.fireside.fm/31)

- Direct and confidential contact with the Fedora Quality Team for quick
  help with issues as they have arisen (not promised, case by case)

- The Fedora Ready [compatibility list](ready/list.xml)

- Regular social media posts and boosts between accounts to support each
  other among our respective communities

## How We Support Vendors {#support}

Fedora Ready support vendors by offering:

- Marketing support through Fedora's channels.

- Technical support for integrating Fedora Linux with their hardware.

- Liaison support to foster community connections.

- Curating feedback from users.

## Vendor and Device Categories {#devices}

- **Fedora preinstalled:** Computers that come with Fedora Linux
  officially preinstalled by the vendor.

- **Fedora supported by vendor:** Computers tested and verified to run
  Fedora Linux successfully.

- **Fedora may run:** Computers that may run Fedora Linux successfully
  based on informal assurances from the vendor.

For example, some Lenovo devices have not been tested for Fedora Linux,
but they were tested for Ubuntu. The Lenovo Linux team has informally
said that Fedora Linux has a decent chance of running on those devices.

## Where Can You Find Fedora Ready Vendors and Devices? {#list}

You can find Vendors and Fedora Ready approved devices on our
[compatibility list](ready/list.xml#fedora-ready-approved-devices).

## How to Contact Us {#contact}

Vendors interested joining the Fedora Ready program can email us at
fedora-ready-team \[at\] fedoraproject \[dot\] org. Then, we will
discuss how to get you onboard.

## Changelog {#changelog}

For added transparency about when changes are made to this document, a
changelog is maintained below.

+----------------------+----------------------+-----------------------+
| Version \#           | Authors              | Description           |
+----------------------+----------------------+-----------------------+
| 1.1                  | Neal Gompa           | Clarify and           |
|                      |                      | genericize the        |
|                      |                      | requirement to allow  |
|                      |                      | any Fedora Edition.   |
+----------------------+----------------------+-----------------------+
| 1.0                  | Roseline Bassey;     | Initial release.      |
|                      | Joseph Gayoso;       | Copyediting led by    |
|                      | Justin Wheeler       | Bassey.               |
+----------------------+----------------------+-----------------------+

: Social Media Contribution Model changelog

# Fedora Ready Brand Guidelines {#_fedora_ready_brand_guidelines}

Roseline Bassey \<<roseline-bassey@fedoraproject.org>\>; Justin W.
Wheeler \<<jww@fedoraproject.org>\>; Fedora community contributors
2025-02-17

**Official Fedora Ready Brand Elements Usage and Guidelines**

This document outlines the proper use of the Fedora [Fedora
Ready](:ready/index.xml) identity elements. It is important to note that
the Fedora Ready brand guidelines are secondary to the [Fedora Trademark
Guidelines](legal::trademarks.xml). Therefore, the Fedora Trademark
Guidelines take precedence over everything outlined in this document.

## What is Fedora Ready Program? {#fedora-ready}

Fedora Ready is a promotional program by the Fedora Project to support
hardware vendors who preload Fedora Linux in their hardware or have
tested their hardware to be compatible with Fedora Linux. The program
aims to also simplify the process for our users to identify and purchase
Fedora Ready approved devices.

## When Should You Use our Guidelines? {#guideline-usage}

Use this guideline if you are working with our brand elements and want
to be sure that you are following the guidelines. If you want to do
something different with the logo, please email fedora-ready-team \[AT\]
fedoraproject \[DOT\] org and we will be happy to help you.

## Brand Messaging {#messaging}

Fedora Ready messaging is clear, friendly, and straightforward. As a
means to promote Fedora Ready vendors and devices, we utilize various
Fedora social channels. Our messaging or content is part of our brand
asset, and it must remain unaltered. If you must use our marketing
content you must provide us the credit. Vendors do not need to imitate
our messaging style since they are free to create their own marketing
messages tailored to their business.

## Permitted Logo Variants {#logo}

The Fedora Ready logo represents the Fedora Ready program and all that
it does. We aim for a simple and modern design that makes our brand
stand out and is easily identifiable.

:::: note
::: title
:::

**Do not use the Fedora Ready logos on this page without written
permission**. The images on this page are Fedora trademarks and are not
licensed for reuse. Please obtain permission for using the Fedora
trademark and follow all applicable guidelines set out on this page and
in the [official trademark guidelines](legal/trademarks.xml).
::::

+----------------------+----------------------+-----------------------+
| ![static](fedora-rea | ![static](fedora     | ![static](fedor       |
| dy/fedora-ready.svg) | -ready/fedora-ready- | a-ready/fedora-ready- |
|                      | monochrome-blue.svg) | monochrome-white.svg) |
| Default logo         |                      |                       |
| (preferred)          | Blue                 | White                 |
+----------------------+----------------------+-----------------------+
| ![sta                | ![sta                | ![s                   |
| tic](fedora-ready/fe | tic](fedora-ready/fe | tatic](fedora-ready/f |
| dora-ready-var2.svg) | dora-ready-var3.svg) | edora-ready-var4.svg) |
|                      |                      |                       |
| Tritone variant 2    | Tritone variant 3    | Tritone variant 4     |
+----------------------+----------------------+-----------------------+

## Obtaining Fedora Ready Logo {#obtaining-logo}

For formal approval on the use of Fedora Ready brand elements, please
follow this process:

- Open a new ticket on [Fedora Council
  repository](https://pagure.io/Fedora-Council/tickets/new_issue) to
  make your request.

- If approved, read this part of the [Fedora logo
  guldeline]({url_wiki}/Logo/UsageGuidelines#Obtaining_the_Fedora_logo_to_your_specifications)
  on how to obtain the logo.

## Using our Logo and Name {#logo-usage}

The \"Fedora Ready\" name and logo are trademarks of Red Hat, Inc.
Please follow the following guidelines for the proper usage of our name
and logo.

## Usage Guidelines

- "Fedora Ready" name and logo should not be used in any context that
  implies Fedora officially endorses a vendor, hardware, product, or
  service or in a way that implies an affiliation with Fedora and/or
  Fedora Ready brand.

- **The Fedora Ready Logo:** The Fedora Ready logo is our primary logo.
  It is made up of Fedora emblem, \"READY\" label, and a gimmick. It
  should only be used on white, black, or gray backgrounds.

- Ensure sufficient contrast between the logo and the background.

- Maintain a clear space around the logo equal to the height of the
  letter \"X\". This ensures other elements do not crowd the logo.

- Our logo should be used in a manner that aligns with our brand values
  and identity.

- To use our logo you need to request formal approval from the Fedora
  Ready team with details of the intended use.

- Provide specifics on where and how the logo will be used and displayed
  to ensure proper evaluation.

### Minimum Size {#logo-size}

To ensure legibility, the logo should not be smaller than 70 pixels in
digital or 20mm in print.

### Incorrect Usage

+-----------------------------------+-----------------------------------+
| ![static]                         | ![sta                             |
| (fedora-ready/stretched-logo.jpg) | tic](fedora-ready/color-logo.png) |
|                                   |                                   |
| Do not distort, stretch, or       | Do not change the colors of the   |
| compress the logo.                | logo.                             |
+-----------------------------------+-----------------------------------+
| ![static](f                       | ![stati                           |
| edora-ready/logo-with-effect.png) | c](fedora-ready/flipped-logo.png) |
|                                   |                                   |
| Do not alter Fedora Ready logo,   | Do not rotate or flip the logo.   |
| or add effects such as shadows,   | Keep the logo's orientation as it |
| gradients, or any other effects.  | is.                               |
+-----------------------------------+-----------------------------------+

## Typography

To maintain consistency across all materials we recommend using the
following fonts. They are both openly licensed and freely shareable:

- **Open Sans**: For body text and copy font.

- **Montserrat**: For titles and headlines.

## Imagery and Graphics {#imagery}

- Use only the product photos provided by the vendor or on the vendor's
  product page.

- Use high-quality, high-resolution product photos or graphics that
  reflect the professionalism of Fedora Ready.

# Fedora Ready Compatibility List {#_fedora_ready_compatibility_list}

Official Fedora Ready compatibility list with hardware and vendors that
work well with Fedora Linux out-of-the-box. 2024-07-24

The Fedora Ready compatibility list includes hardware systems that have
been confirmed to work well with the Fedora operating system. This list
is a part of the [Fedora Ready program](ready/index.xml), which is run
by the Fedora Project to support vendors who ship or provide support for
Fedora Linux on their devices. We keep this list updated to assist our
users in making well-informed decisions when purchasing hardware
intended for use with Fedora Linux.

## Criteria for Vendor Listing {#criteria}

To get your device listed, hardware vendors can check out our [Criteria
for Vendor Listing](ready/index.xml#criteria). Once you have reviewed
it, feel free to [contact us](ready/index.xml#contact) to discuss about
adding your device(s) to the list.

## Device Categories {#categories}

Vendors and devices are categorized into three main groups:

- **Fedora preinstalled:** Computers that come with Fedora Linux
  officially preinstalled by the vendor.

- **Fedora supported by vendor:** Computers tested and verified to run
  Fedora Linux successfully.

- **Fedora may run:** Computers that may run Fedora Linux successfully
  based on informal assurances from the vendor.

## Fedora Ready Vendors

Please find Fedora Ready hardware and vendor specific page below:

- [Lenovo](ready/lenovo.xml)

- [Framework](ready/framework.xml)

- [Slimbook](ready/slimbook.xml)

- [NovaCustom](ready/novacustom.xml)

# Framework {#_framework}

Official Fedora Ready compatibility list with hardware and vendors that
work well with Fedora Linux out-of-the-box. 2024-07-24

Framework Laptop is designed from the outset to be a great Linux laptop.
The [Framework Laptop DIY
Edition](https://frame.work/products/laptop-diy-13-gen-amd) comes with
no OS loaded to let you bring your favorite Linux distribution.

We're also working on enabling firmware updates through LVFS to complete
the Linux experience. Details on Framework Linux laptops are available
at <https://frame.work/linux>

## [Fedora Supported by Vendor](ready/list.xml#categories)

+-------------+-------------+-------------+-------------+-------------+
| Level of    | Device Name | Tested      | Where to    | Notes       |
| Support     |             | Distro      | Purchase    |             |
+=============+=============+=============+=============+=============+
| Fedora      | Framework   | Fedora 40   | [purchase   | [product    |
| supported   | Laptop 13   |             | page](ht    | display     |
| by vendor   | (AMD Ryzen™ |             | tps://frame | pag         |
|             | 7040        |             | .work/produ | e](https:// |
|             | Series)     |             | cts/laptop- | frame.work/ |
|             |             |             | diy-13-gen- | products/la |
|             |             |             | amd/configu | ptop-diy-13 |
|             |             |             | ration/new) | -gen-amd?ta |
|             |             |             |             | b=overview) |
+-------------+-------------+-------------+-------------+-------------+
| Fedora      | Framework   | Fedora 40   | [purchase   | [product    |
| supported   | Laptop 16   |             | p           | display     |
| by vendor   | (AMD Ryzen™ |             | age](https: | pag         |
|             | 7040        |             | //frame.wor | e](https:// |
|             | Series)     |             | k/products/ | frame.work/ |
|             |             |             | laptop16-di | products/la |
|             |             |             | y-amd-7040) | ptop16-diy- |
|             |             |             |             | amd-7040?ta |
|             |             |             |             | b=overview) |
+-------------+-------------+-------------+-------------+-------------+
| Fedora      | Framework   | Fedora 40   | [purchase   | [product    |
| supported   | Laptop 13   |             | page](http  | display     |
| by vendor   | (13th Gen   |             | s://frame.w | page]       |
|             | Intel®      |             | ork/product | (https://fr |
|             | Core™)      |             | s/laptop-di | ame.work/pr |
|             |             |             | y-13-gen-in | oducts/lapt |
|             |             |             | tel/configu | op-diy-13-g |
|             |             |             | ration/new) | en-intel?ta |
|             |             |             |             | b=overview) |
+-------------+-------------+-------------+-------------+-------------+
| Fedora      | Framework   | Fedora 40   | [purchase   |             |
| supported   | Laptop 13   |             | page](h     |             |
| by vendor   | (12th Gen   |             | ttps://fram |             |
|             | Intel®      |             | e.work/prod |             |
|             | Core™)      |             | ucts/refurb |             |
|             |             |             | ished-lapto |             |
|             |             |             | p-diy-12-ge |             |
|             |             |             | n-intel?v=F |             |
|             |             |             | RANDA0016R) |             |
+-------------+-------------+-------------+-------------+-------------+
| Fedora      | Framework   | Fedora 40   | [purchase   | = Lenovo    |
| supported   | Laptop 13   |             | page]       | Official    |
| by vendor   | (11th Gen   |             | (https://fr | Fedora      |
|             | Intel®      |             | ame.work/pr | Ready       |
|             | Core™)      |             | oducts/fact | co          |
|             |             |             | ory-seconds | mpatibility |
|             |             |             | -framework- | list with   |
|             |             |             | laptop-13-d | hardware    |
|             |             |             | iy-edition- | and vendors |
|             |             |             | 11th-gen-in | that work   |
|             |             |             | tel-core?v= | well with   |
|             |             |             | FRANBY0BNC) | Fedora      |
|             |             |             |             | Linux       |
|             |             |             |             | out-        |
|             |             |             |             | of-the-box. |
|             |             |             |             | 2025-11-10  |
|             |             |             |             |             |
|             |             |             |             | Lenovo      |
|             |             |             |             | tests,      |
|             |             |             |             | certifies   |
|             |             |             |             | and         |
|             |             |             |             | supports    |
|             |             |             |             | Linux on    |
|             |             |             |             | several     |
|             |             |             |             | platforms,  |
|             |             |             |             | including   |
|             |             |             |             | the entire  |
|             |             |             |             | ThinkPad,   |
|             |             |             |             | Th          |
|             |             |             |             | inkStation, |
|             |             |             |             | and         |
|             |             |             |             | ThinkCenter |
|             |             |             |             | portfolios. |
|             |             |             |             | These       |
|             |             |             |             | platforms   |
|             |             |             |             | provide     |
|             |             |             |             | globally    |
|             |             |             |             | available   |
|             |             |             |             | Linux       |
|             |             |             |             | preloaded   |
|             |             |             |             | systems     |
|             |             |             |             | that are    |
|             |             |             |             | supported   |
|             |             |             |             | by Lenovo.  |
|             |             |             |             |             |
|             |             |             |             | The goal of |
|             |             |             |             | the program |
|             |             |             |             | is to       |
|             |             |             |             | reduce      |
|             |             |             |             | complexity  |
|             |             |             |             | and offer a |
|             |             |             |             | wider range |
|             |             |             |             | of          |
|             |             |             |             | platforms   |
|             |             |             |             | in order to |
|             |             |             |             | provide     |
|             |             |             |             | Linux users |
|             |             |             |             | with a      |
|             |             |             |             | better user |
|             |             |             |             | experience  |
|             |             |             |             | and         |
|             |             |             |             | confidence  |
|             |             |             |             | that their  |
|             |             |             |             | system will |
|             |             |             |             | run well    |
|             |             |             |             | with Linux. |
|             |             |             |             | Support for |
|             |             |             |             | these       |
|             |             |             |             | platforms   |
|             |             |             |             | is          |
|             |             |             |             | upstream,   |
|             |             |             |             | with        |
|             |             |             |             | firmware    |
|             |             |             |             | updates     |
|             |             |             |             | available   |
|             |             |             |             | through     |
|             |             |             |             | LVFS.       |
|             |             |             |             | Lenovo aims |
|             |             |             |             | to          |
|             |             |             |             | contribute  |
|             |             |             |             | directly to |
|             |             |             |             | the         |
|             |             |             |             | community   |
|             |             |             |             | upstream    |
|             |             |             |             | projects    |
|             |             |             |             | and to      |
|             |             |             |             | continue    |
|             |             |             |             | improving   |
|             |             |             |             | the Linux   |
|             |             |             |             | experience. |
|             |             |             |             |             |
|             |             |             |             | All Fedora  |
|             |             |             |             | p           |
|             |             |             |             | reinstalled |
|             |             |             |             | models have |
|             |             |             |             | been        |
|             |             |             |             | through     |
|             |             |             |             | extensive   |
|             |             |             |             | testing     |
|             |             |             |             | with the    |
|             |             |             |             | Lenovo QA   |
|             |             |             |             | team and    |
|             |             |             |             | appropriate |
|             |             |             |             | regulatory  |
|             |             |             |             | checks. All |
|             |             |             |             | Fedora      |
|             |             |             |             | supported   |
|             |             |             |             | models have |
|             |             |             |             | had Ubuntu  |
|             |             |             |             | ce          |
|             |             |             |             | rtification |
|             |             |             |             | enablement  |
|             |             |             |             | and Lenovo  |
|             |             |             |             | QA testing. |
|             |             |             |             | We expect   |
|             |             |             |             | Fedora to   |
|             |             |             |             | run well on |
|             |             |             |             | them as     |
|             |             |             |             | support is  |
|             |             |             |             | upstream.   |
|             |             |             |             | FW updates  |
|             |             |             |             | are         |
|             |             |             |             | available   |
|             |             |             |             | on LVFS     |
|             |             |             |             |             |
|             |             |             |             | Lenovo is   |
|             |             |             |             | proud to    |
|             |             |             |             | work        |
|             |             |             |             | closely     |
|             |             |             |             | with the    |
|             |             |             |             | Fedora      |
|             |             |             |             | community   |
|             |             |             |             | to provide  |
|             |             |             |             | open source |
|             |             |             |             | friendly    |
|             |             |             |             | solutions.  |
|             |             |             |             | Details on  |
|             |             |             |             | Lenovo      |
|             |             |             |             | Linux       |
|             |             |             |             | platforms   |
|             |             |             |             | are         |
|             |             |             |             | available   |
|             |             |             |             | on          |
|             |             |             |             | www.lenovo  |
|             |             |             |             | .com/linux. |
|             |             |             |             |             |
|             |             |             |             | New (for    |
|             |             |             |             | 2025):      |
|             |             |             |             | Linux       |
|             |             |             |             | disc        |
|             |             |             |             | overability |
|             |             |             |             | on the US   |
|             |             |             |             | and Canada  |
|             |             |             |             | site is     |
|             |             |             |             | finally     |
|             |             |             |             | working:    |
|             |             |             |             |             |
|             |             |             |             | [USA](https |
|             |             |             |             | ://www.leno |
|             |             |             |             | vo.com/us/e |
|             |             |             |             | n/laptops/r |
|             |             |             |             | esults/?vis |
|             |             |             |             | ibleDatas=6 |
|             |             |             |             | 99%3ALinux) |
|             |             |             |             |             |
|             |             |             |             | [Can        |
|             |             |             |             | ada](https: |
|             |             |             |             | //www.lenov |
|             |             |             |             | o.com/ca/en |
|             |             |             |             | /laptops/re |
|             |             |             |             | sults/?visi |
|             |             |             |             | bleDatas=10 |
|             |             |             |             | 68%3ALinux) |
|             |             |             |             |             |
|             |             |             |             | Hoping this |
|             |             |             |             | comes to    |
|             |             |             |             | other geo's |
|             |             |             |             | soon. In    |
|             |             |             |             | Europe and  |
|             |             |             |             | India look  |
|             |             |             |             | for the     |
|             |             |             |             | \"Build     |
|             |             |             |             | Your PC\"   |
|             |             |             |             | option on   |
|             |             |             |             | certified   |
|             |             |             |             | laptops and |
|             |             |             |             | it should   |
|             |             |             |             | have Fedora |
|             |             |             |             | available   |
|             |             |             |             | if the      |
|             |             |             |             | platform    |
|             |             |             |             | has a       |
|             |             |             |             | Fedora      |
|             |             |             |             | preload.    |
|             |             |             |             |             |
|             |             |             |             | []          |
|             |             |             |             | {#fedora-pr |
|             |             |             |             | einstalled} |
|             |             |             |             | == [Fedora  |
|             |             |             |             | Preinst     |
|             |             |             |             | alled](read |
|             |             |             |             | y/list.xml# |
|             |             |             |             | categories) |
|             |             |             |             |             |
|             |             |             |             | \[cols=\"   |
|             |             |             |             | 1,1,1,1\"\] |
+-------------+-------------+-------------+-------------+-------------+

\|Level of Support \|Device Name \|Tested Distro \|Where to Purchase

\|Fedora preinstalled \|Thinkpad X1 Carbon 13th Gen (ARL) \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] - MIPI camera blocked \|Fedora preinstalled \|Thinkpad T16 4th Gen
AMD \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad T14 6th Gen AMD \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad T14 6th Gen Intel \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad P16s 4th Gen AMD \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad P14s 6th Gen AMD \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad X13 6th Gen AMD \|Fedora 42
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Fedora preinstalled \|Thinkpad X1 Carbon 12th Gen \|Fedora 39
\|https://www.lenovo.com/us/en/p/laptops/thinkpad/thinkpadx1/thinkpad-x1-carbon-gen-12-(14-inch-intel)/len101t0083\[USA\]
&
[Canada](https://www.lenovo.com/ca/en/p/laptops/thinkpad/thinkpadx1/thinkpad-x1-carbon-gen-12-(14-inch-intel)/len101t0083)
\|Fedora preinstalled \|Thinkpad X1 Carbon 11th Gen \|Fedora 38
\|https://www.lenovo.com/gb/en/configurator/cto/index.html?bundleId=21HMCTO1WWGB3\[Europe\]
& [North
America](https://www.lenovo.com/us/en/configurator/cto/index.html?orgRef=https%253A%252F%252Fdiscussion.fedoraproject.org%252F&bundleId=21HMCTO1WWUS6)
\|Fedora preinstalled \|Thinkpad T14s 6th Gen AMD \|Fedora 41
\|[USA](https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux)
(also in Canada) \|Fedora preinstalled \|Thinkpad T14 5th Gen AMD
\|Fedora 40
\|[USA](https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux)
(also in Canada) \|Fedora preinstalled \|Thinkpad T14 5th Gen Intel
\|Fedora 40
\|[USA](https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux)
(also in Canada) \|Fedora preinstalled \|Thinkpad P14s 5th Gen AMD
\|Fedora 40
\|[USA](https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux)
(also in Canada) \|Fedora preinstalled \|Thinkpad P1 7th Gen AMD
\|Fedora 40
\|[USA](https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux)
(also in Canada)

\|Fedora preinstalled \|Thinkpad X1 Carbon 12th Gen \|Fedora 39
\|[USA](https://www.lenovo.com/us/en/p/laptops/thinkpad/thinkpadx1/thinkpad-x1-carbon-gen-12-(14-inch-intel)/len101t0083)
&
[Canada](https://www.lenovo.com/ca/en/p/laptops/thinkpad/thinkpadx1/thinkpad-x1-carbon-gen-12-(14-inch-intel)/len101t0083)
\|Fedora preinstalled \|Thinkpad X1 Carbon 11th Gen \|Fedora 38
\|[Europe](https://www.lenovo.com/gb/en/configurator/cto/index.html?bundleId=21HMCTO1WWGB3)
& [North
America](https://www.lenovo.com/us/en/configurator/cto/index.html?orgRef=https%253A%252F%252Fdiscussion.fedoraproject.org%252F&bundleId=21HMCTO1WWUS6)

\|Fedora preinstalled \|ThinkPad Z16 1st Gen \|Fedora 36 \|Lenovo
Europe - build your own PC (e.g. [Configure Your PC, Lenovo
UK](https://www.lenovo.com/gb/en/configurator/cto/index.html?bundleId=21D4CTO1WWGB1))
\|Fedora preinstalled \|ThinkPad Z13 1st Gen \|Fedora 36 \|Lenovo
Europe - build your own PC (e.g. [Configure Your PC, Lenovo
UK](https://www.lenovo.com/gb/en/configurator/cto/index.html?bundleId=21D2CTO1WWGB1))
\|Fedora preinstalled \|ThinkPad P14s 3rd Gen AMD \|Fedora 37 \|Lenovo
Europe - build your own PC (e.g. [Configure Your PC, Lenovo
UK](https://www.lenovo.com/gb/en/configurator/cto/index.html?bundleId=21J5CTO1WWGB1))

+-----------------------------------------------------------------------+
| []{#fedora-supported-by-vendor} == [Fedora Supported by               |
| Vendor](ready/list.xml#categories)                                    |
|                                                                       |
| \[cols=\"1,1,1,1,1\"\]                                                |
+-----------------------------------------------------------------------+

\|Level of Support \|Device Name \|Tested Distro \|Where to Purchase
\|Notes

\|Fedora supported by vendor \|Thinkpad X1 Carbon 13th Gen (LNL)
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|MIPI camera needs RPM Fusion and is not recommended \|Fedora
supported by vendor \|Thinkpad X1 2-in-1 10th Gen \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad T14s 6th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad T14s 2-in-1 1st Gen
Intel \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad T16 4th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad X13 6th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad X9-14 \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|MIPI camera needs RPM Fusion and is not recommended \|Fedora
supported by vendor \|Thinkpad X9-15 \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|MIPI camera need RPM Fusion and is not recommended \|Fedora
supported by vendor \|Thinkpad P16v 3rd Gen Intel \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|Nvidia driver recommended \|Fedora supported by vendor \|Thinkpad
P14s 6th Gen Intel \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad P16s 4th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L13 6th Gen AMD \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L13 2-in-1 6th Gen AMD
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L14 6th Gen AMD \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L16 2nd Gen AMD \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L13 6th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L13 2-in-1 6th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L14 6th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad L16 2nd Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad E14 7th Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|Thinkpad E16 3rd Gen Intel
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P2 Tower 2nd Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P3 Tiny 2nd Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P3 Ultra 2nd Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P3 Tower 2nd Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P8 \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P5 \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation P7 \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkStation PX \|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre Neo30s 5th Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre Neo50q 5th Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre Neo50s 5th Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre Neo50t 5th Gen
\|Fedora 42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M70a 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCenter M90a 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M70q 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M90q 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M70t 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M70s 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \| \|Fedora supported by vendor \|ThinkCentre M90s 6th Gen \|Fedora
42+
\|https://www.lenovo.com/us/en/laptops/results/?visibleDatas=699%3ALinux\[e.g.
US\] \|

\|Fedora supported by vendor \|ThinkPad P16s 1st Gen AMD \|Fedora 37
\|Depends on supply \| \|Fedora supported by vendor \|ThinkPad P1 5th
Gen \|Fedora 37 \| \|Nvidia driver from RPM Fusion recommended \|Fedora
supported by vendor \|ThinkPad P16 1st Gen \|Fedora 37 \| \|Nvidia
driver from RPM Fusion recommended

\|Fedora supported by vendor \|ThinkPad P1 4th Gen \|Fedora 35 \|
\|Nvidia driver from RPM Fusion recommended \|Fedora supported by vendor
\|ThinkPad X1 Carbon 10th Gen \|Fedora 36 \| \|

\|Fedora supported by vendor \|ThinkPad X1 Carbon 9th Gen \|Fedora 33 \|
\| \|Fedora supported by vendor \|ThinkPad P14s AMD 2nd Gen \|Fedora 35
\| \|

+-----------------------------------------------------------------------+
| []{#fedora-may-run} == [Fedora May Run](ready/list.xml#categories)    |
|                                                                       |
| \[cols=\"1,1,1\"\]                                                    |
+-----------------------------------------------------------------------+

\|Level of Support \|Device Name \|Tested Distro

\|Fedora may run \|ThinkPad T470 \|RHEL 7.3 \|Fedora may run
\|ThinkStation P320 \|RHEL 7.3 - 7.x

\|Fedora may run \|ThinkStation P320 Tiny \|RHEL 7.3 - 7.x \|Fedora may
run \|ThinkStation P330 \|RHEL 7.5 - 7.x \|Fedora may run \|ThinkStation
P330 Tiny \|RHEL 7.5 - 7.x, 8.2 - 8.x

\|Fedora may run \|ThinkStation P330 2nd Gen \|RHEL 8.0 - 8.x \|Fedora
may run \|ThinkStation P340 \|RHEL 8.6 \|Fedora may run \|ThinkStation
P340 Tiny \|RHEL 8.5

\|Fedora may run \|ThinkStation P348 \|RHEL 8.5 \|Fedora may run
\|ThinkStation P350 Tiny \|RHEL 8.5 \|Fedora may run \|ThinkStation P350
\|RHEL 8.6

\|Fedora may run \|ThinkStation P358 \|RHEL 8.6 \|Fedora may run
\|ThinkStation P360 Tower \|RHEL 8.6 \|Fedora may run \|ThinkStation
P360 Tiny \|RHEL 8.6

\|Fedora may run \|ThinkStation P360 Ultra \|RHEL 8.6 \|Fedora may run
\|ThinkStation P5 \|RHEL 8.7 \|Fedora may run \|ThinkStation P7 \|RHEL
8.7

\|Fedora may run \|ThinkStation P520 \|RHEL 7.3 - 7.x, 8.6 - 8.x
\|Fedora may run \|ThinkStation P520c \|RHEL 7.3 - 7.x, 8.5 - 8.x
\|Fedora may run \|ThinkStation P620 \|RHEL 8.6

\|Fedora may run \|ThinkStation P720 \|RHEL 7.3 - 7.x, 8.5 - 8.x
\|Fedora may run \|ThinkStation P920 \|RHEL 7.3 - 7.x, 8.5 - 8.x
\|Fedora may run \|ThinkPad Edge A485 \|RHEL 7.5 \|Fedora may run
\|ThinkPad P1 \|RHEL 7.5 - 7.x

\|Fedora may run \|ThinkPad P1 3rd Gen \|RHEL 8.3 \|Fedora may run
\|ThinkPad P14s (Type: 20S4) \|RHEL 8.2 \|Fedora may run \|ThinkPad P14s
AMD (Type: 20Y1) \|RHEL 8.3

\|Fedora may run \|ThinkPad P14s Gen 3 \|RHEL 8.7 \|Fedora may run
\|ThinkPad P15 Gen 1 \|RHEL 8.3 \|Fedora may run \|ThinkPad P15 Gen 2
\|RHEL 8.6

\|Fedora may run \|ThinkPad P15s \|RHEL 8.2 \|Fedora may run \|ThinkPad
P15v \|RHEL 8.3 \|Fedora may run \|ThinkPad P15v Gen 2 \|RHEL 8.6

\|Fedora may run \|ThinkPad P15v Gen 3 \|RHEL 9.1 \|Fedora may run
\|ThinkPad P15v Gen 3 AMD \|RHEL 9.1 \|Fedora may run \|ThinkPad P16s
\|RHEL 9.1

\|Fedora may run \|ThinkPad P17 Gen 1 \|RHEL 8.3 \|Fedora may run
\|ThinkPad P17 Gen 2 \|RHEL 8.6 \|Fedora may run \|ThinkPad P51 \|RHEL
7.3 - 7.x

\|Fedora may run \|ThinkPad P51s \|RHEL 7.3 - 7.x \|Fedora may run
\|ThinkPad P52 \|RHEL 7.5 - 7.x \|Fedora may run \|ThinkPad P52s \|RHEL
7.4 - 7.x

\|Fedora may run \|ThinkPad P43s \|RHEL 8.1 \|Fedora may run \|ThinkPad
P53 \|RHEL 8.1

\|Fedora may run \|ThinkPad P53s \|RHEL 8.0 - 8.x \|Fedora may run
\|ThinkPad P71 \|RHEL 7.3 - 7.x \|Fedora may run \|ThinkPad P72 \|RHEL
7.5 - 7.x \|Fedora may run \|ThinkPad T14 \|RHEL 8.2 - 8.x

\|Fedora may run \|ThinkPad T14 AMD \|RHEL 8.5 \|Fedora may run
\|ThinkPad T14 AMD Gen 2 \|RHEL 8.5 \|Fedora may run \|ThinkPad T14 Gen
2 \|RHEL 8.6

\|Fedora may run \|ThinkPad T14 Gen 3 \|RHEL 9.1 \|Fedora may run
\|ThinkPad T14 Gen 3 AMD \|RHEL 8.7 \|Fedora may run \|ThinkPad T14s
\|RHEL 8.2 - 8.x

\|Fedora may run \|ThinkPad T14s AMD \|RHEL 8.3 \|Fedora may run
\|ThinkPad T14s AMD Gen 2 \|RHEL 8.5 \|Fedora may run \|ThinkPad T14s
Gen 2 \|RHEL 8.4

\|Fedora may run \|ThinkPad T14s Gen 3 \|RHEL 8.6 \|Fedora may run
\|ThinkPad T14s Gen 3 AMD \|RHEL 8.7 \|Fedora may run \|ThinkPad T14s
Gen 4 \|RHEL 9.2

\|Fedora may run \|ThinkPad T15 \|RHEL 8.2 \|Fedora may run \|ThinkPad
T15p \|RHEL 8.3 \|Fedora may run \|ThinkPad T15 Gen 2 \|RHEL 8.5

\|Fedora may run \|ThinkPad T15p Gen 2 \|RHEL 8.5 \|Fedora may run
\|ThinkPad T15p Gen 3 \|RHEL 9.1 \|Fedora may run \|ThinkPad T15g Gen 2
\|RHEL 8.6

\|Fedora may run \|ThinkPad T16 \|RHEL 9.1 \|Fedora may run \|ThinkPad
T16 AMD \|RHEL 8.7

\|Fedora may run \|ThinkPad T470 \|Redhat RHEL 7.3 - 7.x \|Fedora may
run \|ThinkPad T470p \|Redhat RHEL 7.3 - 7.x \|Fedora may run \|ThinkPad
T470s \|Redhat RHEL 7.3 - 7.x

\|Fedora may run \|ThinkPad T480 \|RHEL 7.4 - 7.x \|Fedora may run
\|ThinkPad T480s \|RHEL 7.4 - 7.x \|Fedora may run \|ThinkPad T490
\|RHEL 7.6 - 7.x

\|Fedora may run \|ThinkPad T490s \|RHEL 8.0 - 8.x1 \|Fedora may run
\|ThinkPad T570 \|Redhat RHEL 7.3 - 7.x

\|Fedora may run \|ThinkPad T580 \|RHEL 7.4 - 7.x \|Fedora may run
\|ThinkPad T590 \|RHEL 7.6 - 7.x \|Fedora may run \|ThinkPad X1 Extreme
\|RHEL 7.5 - 7.x

\|Fedora may run \|ThinkPad X1 Extreme 3rd Gen \|RHEL 8.3 \|Fedora may
run \|ThinkPad X1 Extreme 4th Gen \|RHEL 8.5 \|Fedora may run \|ThinkPad
X1 Carbon 5th Gen \|Redhat RHEL 7.3 - 7.x \|Fedora may run \|ThinkPad X1
Carbon 6th Gen \|RHEL 7.4 - 7.x

\|Fedora may run \|ThinkPad X1 Carbon 8th Gen \|RHEL 8.2 \|Fedora may
run \|ThinkPad X1 Carbon 9th Gen \|RHEL 8.4 \|Fedora may run \|ThinkPad
X1 Carbon 10th Gen \|RHEL 8.6

\|Fedora may run \|ThinkPad X1 Carbon 11th Gen \|RHEL 9.2 \|Fedora may
run \|ThinkPad X1 Nano \|RHEL 8.4 \|Fedora may run \|ThinkPad X1 Yoga
3rd \|RHEL 7.4 - 7.x

\|Fedora may run \|ThinkPad X1 Yoga 5th Gen \|RHEL 8.2 \|Fedora may run
\|ThinkPad X1 Yoga 6th Gen \|RHEL 8.4 \|Fedora may run \|ThinkPad X13
\|RHEL 8.2

\|Fedora may run \|ThinkPad X13 Gen 2 \|RHEL 8.4 \|Fedora may run
\|ThinkPad X13 AMD \|RHEL 8.3 \|Fedora may run \|ThinkPad X13 Gen 2 AMD
\|RHEL 8.4

\|Fedora may run \|ThinkPad X13 Gen 3 AMD \|RHEL 8.6 \|Fedora may run
\|ThinkPad X13 Yoga \|RHEL 8.2 \|Fedora may run \|ThinkPad X13 Yoga Gen
2 \|RHEL 8.4

\|Fedora may run \|ThinkPad X270 Redhat \|RHEL 7.3 - 7.x \|Fedora may
run \|ThinkPad X280 \|RHEL 7.4 - 7.x \|Fedora may run \|ThinkPad X380
Yoga \|RHEL 7.4 - 7.x

\|Fedora may run \|ThinkPad X390 \|RHEL 7.6 - 7.x \|Fedora may run
\|ThinkPad X390 Yoga \|RHEL 8.0 - 8.x \|Fedora may run \|ThinkPad Yoga
370 \|Redhat RHEL 7.3 - 7.x

+-----------------------------------------------------------------------+
| = NovaCustom Official Fedora Ready compatibility list with hardware   |
| and vendors that work well with Fedora Linux out-of-the-box.          |
| 2025-05-22                                                            |
|                                                                       |
| NovaCustom builds fully customizable laptops tailored to your exact   |
| needs. Select your CPU, GPU, RAM, storage and even a custom-engraved  |
| chassis. All systems ship with Dasharo coreboot open source firmware  |
| and include an Intel ME "Disabled (HAP-bit)" mode that stops the      |
| Management Engine at boot for maximum privacy.                        |
|                                                                       |
| As a Fedora Ready vendor, NovaCustom ensures full upstream support    |
| and contributes back to [coreboot](https://www.coreboot.org/) and     |
| other open source communities. Learn more at                          |
| <https://novacustom.com/>.                                            |
|                                                                       |
| []{#fedora-preinstalled} == [Fedora                                   |
| Preinstalled](ready/list.xml#categories)                              |
|                                                                       |
| \[cols=\"1,1,1,1\"\]                                                  |
+-----------------------------------------------------------------------+

\|Level of Support \|Device Name \|Tested Distro \|Where to Purchase

\|Fedora preinstalled \|V54 Series \|Fedora 42
\|[Worldwide](https://novacustom.com/product/v54-series/) \|Fedora
preinstalled \|V56 Series \|Fedora 42
\|[Worldwide](https://novacustom.com/product/v56-series/)

+-----------------------------------------------------------------------+
| = Slimbook Official Fedora Ready compatibility list with hardware and |
| vendors that work well with Fedora Linux out-of-the-box. 2024-07-31   |
|                                                                       |
| SLIMBOOK has been manufacturing computers optimized for Linux since   |
| 2015, shipping them globally with exceptional technical support.      |
| Additionally, it engages with the community through local projects,   |
| partnerships with Linux desktops like Fedora, and offers free courses |
| through its \"Linux Center\" academy. Details on Slimbook Fedora      |
| laptops are available at <https://fedora.slimbook.com/>               |
|                                                                       |
| []{#fedora-preinstalled} == [Fedora                                   |
| Preinstalled](ready/list.xml#categories)                              |
|                                                                       |
| \[cols=\"1,1,1,1,1\"\]                                                |
+-----------------------------------------------------------------------+

\|Level of Support \|Device Name \|Tested Distro \|Where to Purchase
\|Notes

\|Fedora preinstalled \|Elemental 14 and 15 Intel i5 1235U \|Fedora 40
\|[Slimbook
Elemental](https://slimbook.com/en/shop/category/laptops-elemental-6)\|
\|Fedora preinstalled \|Elemental 14 and 15 Intel i7 1255U \|Fedora 40
\|[Slimbook
Elemental](https://slimbook.com/en/shop/category/laptops-elemental-6)\|

\|Fedora preinstalled \|Executive 14 and 16 intel i7 13700H \|Fedora 40
\|[Slimbook
Executive](https://slimbook.com/en/shop/category/laptops-executive-8)\|
\|Fedora preinstalled \|HERO intel i7 13629 H \|Fedora 40 \|[Slimbook
HERO](https://slimbook.com/en/shop/category/laptops-hero-9)\|

\|Fedora preinstalled \|Slimbook FEDORA 14 and 16 Intel i7 12700H
\|Fedora 39 \|[fedora.slimbook.com](https://fedora.slimbook.com/)\| No
longer on sale; compatible with Fedora 39 \|Fedora preinstalled \|ProX
14 and 15 AMD Ryzen 7 5700U \|Fedora 39 \|[Slimbook
ProX](https://slimbook.com/en/shop/category/laptops-prox-7)\| No longer
on sale; compatible with Fedora 39

\|Fedora preinstalled \|ProX 15 AMD Ryzen 7 5700U WQHD \|Fedora 39
\|[Slimbook
ProX](https://slimbook.com/en/shop/category/laptops-prox-7)\| No longer
on sale; compatible with Fedora 39 \|Fedora preinstalled \|HERO S intel
i7 11800 H \|Fedora 39 \|[Slimbook
HERO](https://slimbook.com/en/shop/category/laptops-hero-9)\| No longer
on sale; compatible with Fedora 39 \|Fedora preinstalled \|Excalibur 16
AMD Ryzen 7 7840HS \|Fedora 39 \|[Slimbook
Excalibur](https://slimbook.com/en/shop/product/excalibur-16-amd-ryzen-7-7840hs-1267?category=60)\|
No longer on sale; compatible with Fedora 39

# Standard Operating Procedures (SOPs) {#_standard_operating_procedures_sops}

Our formal processes for getting things done.

- [Defining who is officially part of the Marketing
  team]({team_issue_tracker}/-/issues/13)

- [Defining who gets access to our social media
  profiles](https://gitlab.com/fedora/mindshare/home/-/issues/26)

- [Processing event VODs (how we turn the livestreams into separate
  talks)](sops/stream-uploads.xml)

- [Organizing and running team meetings](get-involved.xml#meetings)

  - [Marketing Team Membership
    ^\[🡵\]^](https://gitlab.com/fedora/marketing/marketing-planning/-/issues/13)

  - [Social Media Access
    ^\[🡵\]^](https://gitlab.com/fedora/mindshare/home/-/issues/26)

# Processing Livestreams: A Standardized Workflow {#_processing_livestreams_a_standardized_workflow}

Adrian Edwards \<<moralcode@fedoraproject.org>\>; Justin W. Wheeler
\<<jww@fedoraproject.org>\>; Fedora community contributors

This guide outlines a streamlined procedure for processing Fedora
livestreams (e.g., Flock, release parties, Fedora Week of Diversity),
making it easier to edit and share recordings of these events. This
standardized workflow utilizes tools and automation to minimize manual
effort and ensure consistency.

<div>

::: title
Tools You'll Need
:::

- [LosslessCut](https://www.mifi.no/losslesscut/) (version 3.62.0 or
  later recommended)

- [Sponsorblock](https://sponsor.ajay.app/) (optional, but highly
  recommended)

- [Inkscape](https://inkscape.org/) with the [Next Generator
  extension](https://gitlab.com/Moini/nextgenerator) (for creating
  thumbnails)

- Python scripts (provided in the guide)

</div>

:::: note
::: title
:::

This workflow assumes you have basic familiarity with command-line tools
and video editing concepts.
::::

## Step 1: Prepare Talk Metadata {#preparing-metadata}

This process relies on a CSV file (comma-separated values) containing
information about each talk (title, speakers, identifier). This data is
used for automation and consistency.

### 1.1 Automatic Metadata from Pretalx {#automatic-metadata}

If your event uses Pretalx, you can automatically generate the CSV file
using a Python script:

1.  Clone the repository:
    `git clone https://gitlab.com/fedora/commops/home.git`

2.  Navigate to the script: `cd commops/scripts/conference`

3.  Run the script: `python main.py <Pretalx Schedule URL>`

:::: note
::: title
:::

Ensure the Pretalx schedule accurately reflects the livestream order.
::::

The script can also generate YouTube descriptions based on a template
file (`description.template.txt`).

### 1.2 Manual Metadata Preparation {#manual-metadata}

If you're not using Pretalx, create a CSV file with these columns:

- `Title`: The title of the talk.

- `Speakers`: The speaker(s) of the talk.

- `Identifier`: A unique identifier for the talk (lowercase, no spaces
  or special characters).

Populate the CSV with information for each talk.

## Step 2: Tag Chapters and Breaks {#tagging-segments}

Use Sponsorblock to tag intermissions and chapters in the YouTube video.
This helps automate splitting the video later.

1.  Install the Sponsorblock extension for your browser.

2.  Open the YouTube video.

3.  Use the Sponsorblock controls to tag intermissions (usually
    \"Intermission/Intro Animation\") and optionally tag individual
    talks as \"Chapter.\"

4.  Submit your tags to the Sponsorblock database.

## Step 3: Generate Chapter Timestamps {#generating-timestamps}

YouTube uses a specific format for chapters. You can automate this
formatting using a Python script:

1.  Navigate to the script: `cd commops/scripts/timestamps`

2.  Run the script:
    `python main.py <YouTube video URL> [<stream CSV file>]`

The script will generate the timestamps. If you provide the stream CSV,
it will also fill in the talk titles.

Alternatively, you can manually edit the timestamps from Sponsorblock to
match the YouTube format.

## Step 4: Download and Split the Video {#splitting-video}

Use `yt-dlp` to download the video:

    yt-dlp <YouTube video URL>

Then, use LosslessCut to split the video based on the Sponsorblock tags:

1.  Open LosslessCut and drag the downloaded video into it.

2.  Import the `losslesscut.csv` file (if generated) to load the
    segments.

3.  Verify the segments align with the talks.

4.  Click the export button and configure the settings:

    - Output folder

    - Output file names: `${SEG_LABEL}${EXT}`

    - Enable \"Smart cut (experimental)\"

5.  Click *Export* to split the video.

## Step 5: Create Thumbnails {#creating-thumbnails}

Use Inkscape with the Next Generator extension to create thumbnails for
each talk:

1.  Design your thumbnail template in Inkscape, using variables like
    `%VAR_TalkName%` and `%VAR_PresenterNames%` to represent data from
    your CSV.

2.  Go to **Extensions \> Export \> NextGenerator**.

3.  Configure the export settings:

    - CSV file path

    - File format

    - File name pattern (e.g., `%VAR_Identifier%`)

4.  Click *Export* to generate the thumbnails.

## Step 6: (Optional) Add Thumbnail Intros {#adding-intros}

You can add a short intro with the thumbnail at the beginning of each
talk using a Python script:

1.  Navigate to the script: `cd commops/scripts/transcode`

2.  Prepare a CSV file with talk data (if not already generated).

3.  Configure the `config.ini` file with settings for intro duration,
    etc.

4.  Run the script according to the instructions in the README.

## Areas for Further Automation {#further-automation}

While this workflow incorporates significant automation, some areas can
be further improved:

- **Chapter Creation**: Streamline the process of creating YouTube
  chapters.

- **Recombining Clips**: Automate the handling of interruptions and
  inconsistencies in the livestream.

- **Video Reformatting**: Automatically convert videos to optimal
  formats for processing.

- **Captions**: Explore automated captioning and translation solutions.

- **Audio Processing**: Implement loudness normalization and dynamic
  range compression.

- **End Cards**: Automate the addition of end cards to videos.

These improvements would further enhance the efficiency and robustness
of the livestream processing pipeline.

# Does your team need marketing help? {#_does_your_team_need_marketing_help}

Do you work on a Fedora-related project but can't get people to know it
exists?

- **[Open a ticket in our GitLab]({team_issue_tracker}/-/issues/new)!**

  - **Title must include:** (Team Name) Marketing Ask

  - **Description:** Tell us what your team does and who your team
    leaders are.

    - What is your team looking for?

      - More Contributors?

      - Capture more attention?

  - **Provide a point of contact:** Matrix ID or email

The Marketing Team reviews tickets during our meetings. If your request
is more urgent, let us know in our [Matrix
room]({team_synch_communication}) that you opened a ticket.

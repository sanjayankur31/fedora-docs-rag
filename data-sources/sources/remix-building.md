...
3.  Clone the Fedora Kickstarts repository:
    `git clone https://pagure.io/fedora-kickstarts.git`

4.  Enter the directory: `cd fedora-kickstarts/`

5.  Choose a kickstart to use as a base for your remix

6.  Flatten the kickstart:
    `ksflatten --config [name of kickstart].ks -o flat-[name of kickstart].ks --version [fedora version]`

7.  Edit the `%packages` section of the flat kickstart if you wish to
    add/remove base packages

8.  Set `SELinux` to permissive with: `sudo setenforce 0`

9.  Run `livemedia-creator`:

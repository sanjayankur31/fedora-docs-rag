...
- The boot process fails in the U-Boot console screen with an error
  message mentioning a kernel or initramfs: [???](#entering-grub)
...
macOS or recoveryOS (see the [???](#boot-picker) section) and perform
the following steps to revert to the previous version of m1n1 stage 2
and U-Boot.
...
the steps in [???](#boot-picker) to boot into recoveryOS (preferred) or
macOS, and open a Terminal. Ensure you are connected to the internet,
and then run the {variant-name} installer again, using the instructions
in the [Installation](installation.xml) page (or the steps available on
...
in the [???](#stage1-repair) section to reinstall m1n1 stage 1 and make
the OS bootable again.
...
Next, continue with the steps in the [???](#dfu-fedora) section (you
have already performed steps 1-3 and do not need to do them again).
...
- **Spotify**: Follow the instructions in the [???](#widevine) section,
  and then use the web version.
